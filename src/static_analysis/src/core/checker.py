...
def get_path_text(G, path, caller=None):
    ...
    for node in path:
        ...
        file_path = G.get_ast_node_file_path(node)
        ...
        cur_path_str2 += "$FilePath${}\n".format(G.get_ast_node_file_path(node))
        try:
        ...
        if content is not None:
            cur_path_str2 += "Line {}\t{}".format(start_lineno, "".join(content[start_lineno : end_lineno + 1]))
...
def vul_checking(G, pathes, vul_type):
    ...
    """
    sink_funcs = Sinks()
    sanitation_funcs = sink_funcs.get_sinks_by_vul_type("sanitation", add_sinks=False)
    xss_rule_lists = [
        [
            ("has_user_input", None),
            ("not_start_with_func", ["sink_hqbpillvul_http_write"]),
            ("not_exist_func", ["parseInt"]),
            ("end_with_func", sink_funcs.get_sinks_by_vul_type("xss")),
        ],
    ]
    ...
    proto_pollution = [[("has_user_input", None), ("not_exist_func", sanitation_funcs)]]
    path_traversal = [
        [
    ...
    output_rules = [[("not_start_within_file", ["child_process.js"])]]
