        ...
        if G.get_node_attr(node)["type"] == "AST_PARAM_LIST":
            ...
            for param_id in params_id:
                ...
                try:
                    ...
                    loggers.main_logger.error("ERROR: " + str(G.get_node_attr(param_id)) + " AST null type at core.helpers line 53")
                    return []
        ...
        for idx in range(min(len(callee_paras), len(caller_para_names))):
            ...
            added_edge_list.append(
                (
                    CPG_caller_id,
                    callee_paras[idx],
                    {"type:TYPE": "INTER_FUNC_REACHES", "var": str(caller_para_names[idx])},
                )
            )
...
def parse_file(G: Graph, path, start_node_id=0):
    """
    ...
    """
    result = esprima_parse(path, ["-n", str(start_node_id), "-o", "-"], print_func=loggers.main_logger.info)
    G.import_from_string(result)


def parse_string(G: Graph, source, start_node_id=0):
    """
    ...
    Args:
        ...
        start_node_id (int) 0: the start node id
    """
    result = esprima_parse("-", ["-n", str(start_node_id), "-o", "-"], input=source, print_func=loggers.main_logger.info)
    G.import_from_string(result)


def print_handle_result(handle_result: NodeHandleResult):
    output = (
        f"{handle_result.ast_node} "
        f"handle result: obj_nodes={handle_result.obj_nodes}, "
        f"name={handle_result.name}, name_nodes={handle_result.name_nodes}, ast_node={handle_result.ast_node}"
    )
...
def peek_variables(G: Graph, ast_node, handling_func: Callable, extra: ExtraInfo):
    """
...
def add_contributes_to(G: Graph, sources, target, operation: str | None = None, index: int | None = None, rnd: str | None = None, chain_tainted=True):
    """
    ...
    for i, source in enumerate(sources):
        ...
        loggers.main_logger.debug(
            f"Object {_source}({source_name}) " f"CONTRIBUTES TO {target}({target_name}) " f"(Operation: {operation}, Index: {index or i}), tainted: {tainted}"
        )
...
def analyze_json(G, json_str, start_node_id=0, extra=None):
    ...
    result = esprima_parse("-", ["-n", str(start_node_id - 8), "-o", "-"], input=json_str, print_func=logger.info)
    G.import_from_string(result)
