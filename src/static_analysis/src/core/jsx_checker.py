        ...
        if not ast_node:
            ...
            if node_attr["type"] == "jsx" or JSX_Label_Key in node_attr:
                ...
                if not ast_node:
                    ...
                    loggers.validate_logger.info(f"[Error] can not find ast nodes for JSX node {node}")
        if ast_node:
...
def jsx_get_path_text(G: Graph, ast_path, caller=None):
    ...
    for node in ast_path:
        ...
        if os.path.isfile(options.input_file):
            ...
            node_file_path = G.get_ast_node_file_path(node)
...
def clean_and_merge_paths(all_paths):
    # Step 1: Normalize each path by:
    #   - removing consecutive duplicates
    #   - ensuring order is preserved
    #   - converting to tuple for hashing
    normalized_paths = []
    for path in all_paths:
        ...
        seen = set()
        for step in path:
            if not deduped or step != deduped[-1]:  # remove consecutive duplicates
                deduped.append(step)
        normalized_paths.append(tuple(deduped))

    # Step 2: Sort paths by descending length
    normalized_paths.sort(key=lambda x: -len(x))

    # Step 3: Filter out subpaths
    final_paths = []
    for i, candidate in enumerate(normalized_paths):
        if not any(is_subpath(candidate, other) for other in final_paths):
            final_paths.append(candidate)

    return [list(path) for path in final_paths]


def is_subpath(short, long):
    """Returns True if `short` is an ordered subsequence of `long`."""
...
def jsx_traceback(G: Graph, vul_type, start_node=None):
    ...
    for start_node in start_node_list:
        pathes = G._dfs_upper_by_multi_edge_types(
            start_node,
            ["CONTRIBUTES_TO", JSX_Edges.JSX_DATA_FLOW, JSX_Edges.JSX_PARENT_OF, "OBJ_TO_PROP", "NAME_TO_OBJ"],
        )
        for path in pathes:
...
def jsx_vul_checking(G: Graph, pathes, vul_type):
    ...
    if G.check_a_href:
        xss_rule_lists.append(
            [
                ("jsx_has_user_input", None),
                ("jsx_has_dom_xss_pair", None),
                ("jsx_has_no_indirect_src", None),
                ("jsx_not_sanitized", None),
            ]
        )
    ...
    if G.check_window_open:
        xss_rule_lists.append(
            [("jsx_has_user_input", None), ("jsx_not_sanitized", None), ("jsx_has_callee_of_dom_op", None)]
        )

    vul_type_map = {"xss": xss_rule_lists}
...
def jsx_print_success_pathes(G, success_pathes, color=None):
    ...
    color_map = {"green": sty.fg.li_green, "red": sty.fg.li_red, "blue": sty.fg.li_blue, "yellow": sty.fg.li_yellow}
