...
class ColorFormatter(logging.Formatter):
    def __init__(self):
        ...
        datefmt = "%Y-%m-%d %H:%M:%S"  # Local time in readable format
...
class Loggers:
    class __Loggers:
        ...
        def update_loggers(self, base_location=options.log_base_location):
            ...
            if options.print:
                ...
                self.main_logger = create_logger("main", file_name=os.path.join(base_location, "run_log.log"))
            ...
            self.eval_logger = create_logger("eval", file_name=os.path.join(base_location, "eval.log"))
            self.code_slice_logger = create_logger("code_slice", file_name=os.path.join(base_location, "code_slice.log"))
            self.validate_logger = create_logger("validate", file_name=os.path.join(base_location, "validate.log"))
    ...
    tmp_res_logger: Logger
    eval_logger: Logger
    code_slice_logger: Logger
    validate_logger: Logger

    instance: Union[None, __Loggers] = None
