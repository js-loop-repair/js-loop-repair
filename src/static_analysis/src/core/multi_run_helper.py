...
def validate_package(package_path):
    ...
    return os.path.exists(package_json_path) or os.path.exists(js_path) or os.path.exists(ts_path)


def validate_jsx_package(package_path):
    """
    Check whether a package is valid by whether it includes package.json, index.js, index.jsx, index.ts, or index.tsx.
    ...
    js_path = os.path.join(package_path, "index.js")
    jsx_path = os.path.join(package_path, "index.jsx")
    ts_path = os.path.join(package_path, "index.ts")
    tsx_path = os.path.join(package_path, "index.tsx")

    return (
        os.path.exists(package_json_path)
        or os.path.exists(js_path)
        or os.path.exists(jsx_path)
        or os.path.exists(ts_path)
        or os.path.exists(tsx_path)
    )
...
def get_entrance_files_of_jsx_package(package_path: str, get_all=False, options=None) -> List[str]:
    def get_src_file_path(path: str) -> str:
        if path.startswith(("lib/index", "build/index")):
            path = "src/index"
        return path

    def get_next_js_pages(package_path):
        # Define paths for both 'pages/' and 'src/pages/'
        pages_paths = [os.path.join(package_path, "pages"), os.path.join(package_path, "src", "pages")]
        entrance_files = []
                ...
                for root, dirs, files in os.walk(path):
                    for file in files:
                        if file.endswith((".js", ".jsx")):
                            entrance_files.append(os.path.join(root, file))

        return entrance_files

    def is_next_js_app(package_data):
        is_next_js_app = any(package_data.get("dependencies", {}).get(dep) for dep in ["next", "gatsby", "umi"])
        if options:
            options.is_nextjs_application = is_next_js_app
        return is_next_js_app
    ...
    with open(package_json_path, "r") as f:
        try:
            ...
            print("reading package.json errror", e)
            loggers.main_logger.error("reading package.json error")
            return []
        ...
        for key in ["module", "browser", "main"]:
            ...
            if isinstance(value, str):
                src_file_path = get_src_file_path(value)
                if src_file_path == "build":
                    common_entrances.append("build/index")
                elif src_file_path == "src":
                    common_entrances.append("src/index")
                else:
                    common_entrances.append(src_file_path)
                break

        common_entrances.extend(
            [
                "src/index",
                "index",
                "src/app",
        ...
        entrance_candidates.extend(
            os.path.join(package_path, entry + ("" if entry.endswith((".js", ".jsx")) else ext))
            for entry in common_entrances
            for ext in [".js", ".jsx"]
        )

        if is_next_js_app(package_data):
            entrance_candidates.extend(get_next_js_pages(package_path))

        entrance_candidates.append(os.path.join(package_path, "src/contentScripts/prepareInjection.js"))

        for candidate in entrance_candidates:
            normalized_path = os.path.normpath(candidate)
            if os.path.exists(normalized_path) and normalized_path not in entrance_files:
                entrance_files.add(normalized_path)

    return list(entrance_files)
...
def get_entrance_files_of_package(package_path, get_all=False):
    ...
    with open(package_json_path) as fp:
        ...
        try:
            ...
            loggers.main_logger.error("Special {}".format(package_path))
    ...
    if get_all:
        ...
        proc = subprocess.Popen(
            ["node", analysis_path, package_path], text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
def test_file(file_path, vul_type="xss", graph=None):
    ...
    try:
        if vul_type == "proto_pollution":
            ...
            G = unittest_main(test_file_name, vul_type=vul_type, args=args, check_signatures=get_all_sign_list())
...
def main():
    ...
    if args.print:
        create_logger("main_logger", output_type="console", level=logging.DEBUG)
        create_logger("graph_logger", output_type="console", level=logging.DEBUG)
        create_logger("npmtest", output_type="console", level=logging.DEBUG)
    ...
    npm_success_logger = create_logger(
        "npmsuccess", output_type="file", level=10, file_name="npmsuccess_{}.log".format(vul_type)
    )
    ...
    npm_test_logger.info(
        "Success rate: {}%, {} out of {}, {} skipped and {} failed".format(
            float(len(success_list)) / total_cnt * 100,
            len(success_list),
            total_cnt,
            len(skip_list),
            total_cnt - len(skip_list) - len(success_list),
        )
    )
    npm_test_logger.info(
        "{} fails caused by package error, {} fails caused by generate error".format(
            len(not_found), len(generate_error)
        )
    )
