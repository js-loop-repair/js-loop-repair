...
from .validate_checker import validate_traceback, slice_traceback, CheckStatus
from .multi_run_helper import (
    validate_jsx_package,
    validate_package,
    get_entrance_files_of_package,
    get_entrance_files_of_jsx_package,
)
...
class OPGen:
    ...
    def generate_opg_graph_for_avr(self, G: Graph, vul_type="os_command", second_start_node=None, generate_new_graph=True):
        """
        ...
        loggers.validate_logger.info(f"entry_id:  {entry_id}")
        if options.service_entry and G.service_registry:
            for route in G.service_registry:
                loggers.main_logger.info(f"calling service registry: {route}")
        ...
        print(f"Number of Graph Nodes: {len(G.get_all_nodes())}")

        loggers.code_slice_logger.info(f"G.package_name: {G.package_name}")
    ...
    def _test_graph(self, G: Graph, vul_type="os_command", second_start_node=None):
        ...
        if options.export:
            ...
            G.export_to_CSV(exported_nodes_file_path, exported_rels_file_path, light=False)
        ...
        end_time = time.time()
        duration = end_time - start_time
        loggers.eval_logger.info(json.dumps({"duration_of_query": f"{duration:.2f}"}))
        print(f"Checking vulnerabilities took {duration:.2f} seconds")
        return check_res

    def generate_ast_graph_for_avr(
        self,
        module_path,
        vul_type="xss",
        G: Graph | None = None,
        ...
        if module_path is None:
            print(sty.fg.li_red + sty.ef.inverse + "[ERROR] {} not found".format(module_path) + sty.rs.all)
        ...
        if generate_new_graph:
            ...
            G = self.prepare_graph_for_new_file(package_name=module_path)

        if options.is_nextjs_application:
            options.is_jsx_application = False
        if options.is_jsx_application:
            parse_file(G, module_path)
        else:
            if options.nodejs:
                js_call_templete = "var main_func=require('{}');".format(module_path)
                loggers.validate_logger.info(f"js_call_templete:  {js_call_templete}")
            else:
                js_call_templete = f"import * as entry from '{module_path}'"
            try:
                start_node_id = 0
                if (not generate_new_graph) and G.num_of_ast_nodes:
                    start_node_id = G.num_of_ast_nodes
                parse_string(G, js_call_templete, start_node_id=start_node_id)
            except:
                return []

    def test_module(self, module_path, vul_type="xss", G=None, timeout_s=None, from_nodejs=False):
        """
        test a file as a module
        Args:
            module_path: the path to the module
    ...
    def test_nodejs_package(self, package_path, vul_type="os_command", G=None, timeout_s=None):
        """
        ...
        if not validate_package(package_path):
            print(sty.fg.li_red + sty.ef.inverse + "[ERROR] {} not found".format(package_path) + sty.rs.all)
            return -1
        if G is None:
            G = self.graph
    ...
    def analyze_code_for_AVR(
        ...
        diff_file_list = self.get_file_lists_by_diff_list(diff_list, package_path)
        options.diff_file_list = diff_file_list
        diff_file_list.sort()

        if options.use_diff_files_as_entrance:
            entrance_files = diff_file_list

        files_to_be_analyzed = []
        combined = entrance_files + diff_file_list
        seen = set()

        for item in combined:
            if item not in seen:
                files_to_be_analyzed.append(item)
                seen.add(item)

        if validate and not files_to_be_analyzed and G.package_name:
            raise ValueError("validate=True and no entrance files found.")

        loggers.main_logger.info(f"Package started: {G.package_name}")
        loggers.main_logger.info(f"Files to be analyzed: {files_to_be_analyzed}")
        loggers.validate_logger.info(f"Entrance Files: {entrance_files}")

        if not files_to_be_analyzed:
            raise ValueError(f"can not find entry point for {package_path}")

        entrance_files.sort()
        loggers.validate_logger.info(f"entrance_files here:  {entrance_files}")
        loggers.main_logger.info(f"entrance_files:  {entrance_files}")
        entrance_files.reverse()
        self.generate_graph_for_avr_with_diff_list(
            entrance_files,
            files_to_be_analyzed,
            package_path,
            diff_list,
    ...
    # TODO: deprecated
    def get_method_name_from_diff_list_method_change(self, method_change_name):
        if method_change_name.startswith("exports."):
            return method_change_name[len("exports.") :]
        return method_change_name

    def get_file_lists_by_diff_list(self, diff_list, package_path):
        diff_file_list = []

        if diff_list:
            if os.path.isfile(package_path):
                diff_file_list.append(package_path)
            else:
                for diff_item in diff_list:
                    old_path = diff_item["old_path"]
                    absolute_old_path = os.path.join(package_path, old_path)
                    diff_file_list.append(absolute_old_path)

        return diff_file_list

    def generate_graph_for_avr_with_diff_list(
        self,
        entrance_files,
        files_to_be_analyzed,
        package_path,
        ...
        if not entrance_files:
            raise ValueError("Can not find the entrance files for generating graph.")
        loggers.code_slice_logger.info(f"entrance_files:  {entrance_files}")

        for entrace_file_index, entrance_file in enumerate(entrance_files):
            if G.finished:
                break
            generate_new_graph = True
            if entrace_file_index > 0:
                generate_new_graph = False

            loggers.code_slice_logger.info(f"generating ast graph for {entrance_file}")
            self.generate_ast_graph_for_avr(entrance_file, vul_type, G, timeout_s=timeout_s, from_nodejs=True, generate_new_graph=generate_new_graph)
            G = self.graph

        ## export AST graph first
        if options.export:
            loggers.main_logger.info("start exporting graph to csv")
            exported_nodes_file_path = os.path.join(options.log_base_location, "nodes.csv")
            exported_rels_file_path = os.path.join(options.log_base_location, "rels.csv")
        ...
        # generate ODGs for these AST graphs
        G.entrance_files = entrance_files
        loggers.code_slice_logger.info(f"G.entrance_files:  {entrance_files}")
        loggers.validate_logger.info(f"G.entrance_files:  {entrance_files}")

        # check if the files in the diff list are already analyzed. if not, generate AST Graphs for all the diff_list files. And also log the reason for further analysis.
        # by finding the name space
        file_nodes = self.graph.get_nodes_by_type_and_flag("AST_TOPLEVEL", "TOPLEVEL_FILE")
        loggers.code_slice_logger.info(f"file_nodes:  {file_nodes}")

        analyzed_files = set()
        for file_node in file_nodes:
            node_attr = self.graph.get_node_attr(file_node)
            file_name = node_attr.get("name")
            analyzed_files.add(file_name)

        for analyzed_file in analyzed_files:
            loggers.code_slice_logger.info(f"analyzed_file:  {analyzed_file}")

        unanalyzed_files = []
        for f in files_to_be_analyzed:
            if f not in analyzed_files:
                unanalyzed_files.append(f)

        if unanalyzed_files:
            raise ValueError(f"The following files are not analyzed: {unanalyzed_files}")
            loggers.error_logger.warning(f"The following files are not analyzed: {unanalyzed_files}")

        # generate ODGs for all the diff_list files
        if timeout_s is not None:
            try:
                func_timeout(timeout_s, self.generate_opg_graph_for_avr, args=(G, vul_type, None))
            except FunctionTimedOut:
                error_msg = "timedout after {} seconds".format(timeout_s)
                loggers.error_logger.error(error_msg)
                raise
            except Exception as e:
                loggers.error_logger.error(f"An error occurred: {e}")
            ...
            self.generate_opg_graph_for_avr(
                G,
                vul_type=vul_type,
                second_start_node=None,
            )
            print(f"number of ast node: {G.num_of_ast_nodes}")

        if options.slice and diff_list:
            for diff in diff_list:
                old_path = diff["old_path"]
                loggers.code_slice_logger.info(f"start generating code slice for {old_path}")
                full_path = os.path.join(options.avr_app_path, old_path)
                diff_parsed = diff["diff_parsed"]
                deleted = diff_parsed["deleted"]
                added = diff_parsed["added"]
                method_change = diff["method_change"]
                loggers.code_slice_logger.info(f"delete lines {deleted}")
                loggers.code_slice_logger.info(f"length of method_change: {len(method_change)}")
                visited_deleted_line_func_ast_id = set()

                for m in method_change:
                    start_line = m["start_line"]
                    loggers.code_slice_logger.info(f"start_line:  {start_line}")
                    end_line = m["end_line"]

                    loggers.code_slice_logger.info(f"added line range to pending code slice for method change {start_line} {end_line} {full_path}")
                    # G.add_line_range_to_pending_code_slice(full_path, start_line, end_line)

                    change_method_AST_nodes = self.graph.get_ast_nodes_by_file_path_and_line_number(full_path, start_line)
                    loggers.code_slice_logger.info(f"change_method_AST_nodes:  {change_method_AST_nodes}")
                    for delete_line_ast_node in change_method_AST_nodes:
                        is_self = False
                        delete_line_ast_node_id = delete_line_ast_node[0]
                        delete_line_ast_node_attr = G.get_node_attr(delete_line_ast_node_id)
                        if delete_line_ast_node_attr.get("type") in ["AST_FUNC_DECL", "AST_CLOSURE", "AST_METHOD"]:
                            delete_line_func_ast_id = delete_line_ast_node_id
                            is_self = True
                        else:
                            delete_line_func_ast_id = delete_line_ast_node[1].get("funcid:int")
                        loggers.code_slice_logger.info(f"delete_line_func_ast_id:  {delete_line_func_ast_id}")
                        ast_node_attr = G.get_node_attr(delete_line_func_ast_id)
                        if ast_node_attr.get("type") == "AST_TOPLEVEL":
                            continue
                        if delete_line_func_ast_id is None:
                            raise ValueError(f"Can not find the funcid:int for deleted line {full_path} {start_line}")
                        if delete_line_func_ast_id in visited_deleted_line_func_ast_id:
                            loggers.code_slice_logger.info(f"skip {delete_line_func_ast_id}")
                            continue
                        if not is_self:
                            if G.get_node_attr(delete_line_func_ast_id).get("funcid:int") in visited_deleted_line_func_ast_id:
                                loggers.code_slice_logger.info(f"skip {delete_line_func_ast_id}")
                                continue

                        deleted_line_func_ast_attr = G.get_node_attr(delete_line_func_ast_id)
                        func_start_line = deleted_line_func_ast_attr.get("lineno:int")
                        func_end_line = deleted_line_func_ast_attr.get("endlineno:int")
                        if int(func_start_line) == int(start_line) and int(func_end_line) == int(end_line):
                            loggers.code_slice_logger.info(f"method_change func ast id:  {delete_line_func_ast_id}")
                            G.add_ast_node_to_pending_code_slice(delete_line_func_ast_id)
                            G.add_ast_node_to_taint_function_list(delete_line_func_ast_id)
                            G.backward_forward_analysis_by_functions(delete_line_func_ast_id)
                            visited_deleted_line_func_ast_id.add(delete_line_func_ast_id)

                # visited_deleted_line_func_ast_id = set()
                for d in deleted:
                    deleted_line_number = d[0]
                    G.add_line_range_to_pending_code_slice(full_path, deleted_line_number, deleted_line_number)
                    loggers.code_slice_logger.info(f"added line range to pending code slice for deleted line {deleted_line_number} {full_path}")
                    deleted_line_ast_nodes = self.graph.get_ast_nodes_by_file_path_and_line_number(full_path, deleted_line_number)
                    for delete_line_ast_node in deleted_line_ast_nodes:
                        delete_line_func_ast_id = delete_line_ast_node[1].get("funcid:int")
                        if not delete_line_func_ast_id:
                            loggers.code_slice_logger.warning(f"delete_line_func_ast_id:  {delete_line_func_ast_id} : {delete_line_ast_node[1]}")
                            continue
                            # raise ValueError(f"Can not find the funcid:int for deleted line {full_path} {deleted_line_number}")
                        if delete_line_func_ast_id in visited_deleted_line_func_ast_id:
                            continue
                        loggers.code_slice_logger.info(f"delete_line_func_ast_id:  {delete_line_func_ast_id}")
                        delete_line_func_ast_node_attr = G.get_node_attr(delete_line_func_ast_id)

                        loggers.code_slice_logger.info(f"delete_line_func_ast_node_attr:  {delete_line_func_ast_node_attr}")
                        if delete_line_func_ast_node_attr.get("funcid:int") in visited_deleted_line_func_ast_id:
                            continue
                        if delete_line_func_ast_node_attr.get("type") == "AST_TOPLEVEL":
                            continue
                        visited_deleted_line_func_ast_id.add(delete_line_func_ast_id)
                        loggers.code_slice_logger.info(f"deleted line func AST id {delete_line_func_ast_id}")
                        G.add_ast_node_to_pending_code_slice(delete_line_func_ast_id)
                        G.add_ast_node_to_taint_function_list(delete_line_func_ast_id)
                        G.backward_forward_analysis_by_functions(delete_line_func_ast_id)
                if not deleted or len(deleted) == 0:
                    for d in added:
                        loggers.code_slice_logger.info("using added lines")
                        deleted_line_number = d[0]
                        G.add_line_range_to_pending_code_slice(full_path, deleted_line_number, deleted_line_number)
                        deleted_line_ast_nodes = self.graph.get_ast_nodes_by_file_path_and_line_number(full_path, deleted_line_number)
                        for delete_line_ast_node in deleted_line_ast_nodes:
        ...
        if options.slice:
            taint_function_list = G.generate_taint_function_list()
            G.vul_graph_info["taint_function_list"] = taint_function_list

            if options.slice_cfg:
                for taint_function in taint_function_list:
                    ...
                    loggers.code_slice_logger.info(f'taint function {taint_function.get("id")} calling {taint_func_obj_nodes}')
                    if taint_func_obj_nodes:
                        result = call_function(G, taint_func_obj_nodes, mark_fake_args=True)
                        loggers.code_slice_logger.info(f"result:  {result}")

                for taint_function in taint_function_list:
                    taint_func_ast_id = taint_function.get("id")
                    loggers.code_slice_logger.info(f"taint_func_ast_id:  {taint_func_ast_id}")
                    taint_func_obj_nodes = G.get_obj_nodes_by_ast_node(taint_function.get("id"), aim_type="function")
                    loggers.code_slice_logger.info(f"taint_func_obj_nodes:  {taint_func_obj_nodes}")

                    all_child_nodes = G.get_all_child_nodes(taint_func_ast_id)
                    all_calls_functions = []
                    for all_child_node in all_child_nodes:
                        calls_edges = G.get_out_edges(taint_func_ast_id, edge_type="CALLS")
                        for e in calls_edges:
                            if e[1] and G.get_node_attr(e[1]).get("type") == "AST_CLOSURE":
                                all_calls_functions.append(e[1])
                    loggers.code_slice_logger.info(f"all_calls_functions:  {all_calls_functions}")
                    all_calls_functions = list(set(all_calls_functions))
                    for a in all_calls_functions:
                        file_path = G.get_ast_node_file_path(a)
                        if not file_path:
                            continue
                        G.pending_code_slice.add_ast_node(file_path, ast_node_id=a)

            G.generate_source_function_list()
            slice_traceback(G, diff_list)
            vul_paths = G.vul_graph_info.get("vul_paths", [])
            G.generate_code_slice()

            if len(vul_paths) == 0:
                G.pending_code_slice.file_path_and_source_function_nodes = G.pending_code_slice.file_path_and_taint_function_nodes

        if options.validate:
            security_check, functionality_check, path_validation_map, return_type_check = validate_traceback(G, diff_list)
            loggers.validate_logger.info(f"path_validation_map:  {path_validation_map}")
            json_safe_map = {key: {subkey: subval.name for subkey, subval in val.items()} for key, val in path_validation_map.items()}
            check_result = {
                "security_check": security_check.value,
                "functionality_check": functionality_check.value,
                "path_validation_map": json_safe_map,
                "return_type_check": return_type_check.value,
            }
            with open(options.validation_output_path, "w", encoding="utf-8") as f:
                json.dump(check_result, f)
                print(f"validation result written to {options.validation_output_path}")
    ...
    def prepare_graph_for_new_file(self, package_name=None):
        if not package_name:
            self.graph.package_name = options.input_file
        else:
    ...
    def output_args(self):
        ...
        for key in keys:
            loggers.main_logger.info("{}: {}".format(key, options.instance.__dict__[key]))
    ...
    def run(self):
        ...
        if not options.diff_list and not (options.validate):
            raise ValueError("diff list is not provided")

        if options.repair_diff_path:
            with open(options.repair_diff_path, "r", encoding="utf-8") as f:
                options.repair_diff = json.load(f)

        if options.diff_list:
            with open(options.diff_list, "r", encoding="utf-8") as f:
        ...
        if options.list is not None:
            ...
            for package_path in package_list:
                ...
                self.test_nodejs_package(package_path, options.vul_type, self.graph, timeout_s=timeout_s)

                if len(self.graph.detection_res[options.vul_type]) != 0:
                    loggers.succ_logger.info("{} is detected in {}".format(options.vul_type, package_path))
                else:
                    loggers.res_logger.info("Not detected in {}".format(package_path))

        else:
            if options.validate:
                self.analyze_code_for_AVR(
                    options.input_file,
                    diff_list,
                    options.vul_type,
                    G=self.graph,
                    timeout_s=timeout_s,
                    is_jsx_application=options.is_jsx_application,
                    validate=True,
                )
            elif diff_list:
                self.analyze_code_for_AVR(
                ...
                self.test_nodejs_package(options.input_file, options.vul_type, G=self.graph, timeout_s=timeout_s)
            else:
            ...
            #         + sty.rs.all
            #     )
            #     loggers.succ_logger.info("{} is detected in {}".format(options.vul_type, options.input_file))
            # else:
            #     loggers.res_logger.info("Not detected in {}".format(options.input_file))
        ...
        if options.export:
            ...
            exported_nodes_file_path = os.path.join(options.log_base_location, "nodes.csv")
            exported_rels_file_path = os.path.join(options.log_base_location, "rels.csv")
            vul_graph_info_file_path = os.path.join(options.log_base_location, "vul_graph_info.json")
            self.graph.export_to_CSV(exported_nodes_file_path, exported_rels_file_path, light=False, vul_graph_info_file_path=vul_graph_info_file_path)
            loggers.main_logger.info("graph exported to csv")
...
def generate_obj_graph(G: Graph, internal_plugins, entry_nodeid="0", OPGen=None):
    ...
    Args:
        ...
        entry_nodeid (str) 0: the entry AST node id,
            by default 0
    """
    from src.plugins.internal.handlers.functions import call_function, run_exported_functions, ast_call_function
    ...
    for index, entrance_file in enumerate(G.entrance_files):
        for node in toplevel_nodes:
            toplevel_node_name = G.get_node_attr(node).get("name")
            if toplevel_node_name == "stdin" and index > 0:
                loggers.validate_logger.info(f"entrance_file node id:  {node}")
                internal_plugins.dispatch_node(node)

    loggers.validate_logger.info(f"entry_nodeid:  {entry_nodeid}")
    ...
    if not options.is_jsx_application:
        ...
        toplevel_nodes = G.get_nodes_by_type_and_flag("AST_TOPLEVEL", "TOPLEVEL_FILE")
        loggers.validate_logger.info(f"toplevel_nodes:  {toplevel_nodes}")
        # loggers.code_slice_logger.info(f"toplevel_nodes: {toplevel_nodes}")
        module_exports_objs = []
        found = False
        for node in toplevel_nodes:
            toplevel_node_name = G.get_node_attr(node).get("name")
            if toplevel_node_name == file_path:
                found = True
            node_attr = G.get_node_attr(node)
            saved_module_exports = node_attr.get("module_exports")
            if saved_module_exports != None:
                module_exports_objs.extend(saved_module_exports)
                # break
            else:
                continue
                ...
                if prototypes:
                    for p in prototypes:
                        ...
                        call_function(
                            ...
                            args=[],
                            is_new=False,
                            this=NodeHandleResult(obj_nodes=[this_obj]),
                            mark_fake_args=True,
                        )
    ...
    end_time = time.time()
    duration = end_time - start_time
    loggers.eval_logger.info(json.dumps({"duration_of_snapshot": f"{duration:.6f}", "jsx_nodes": len(G.get_all_jsx_component_nodes())}))
    start_time = time.time()
    ...
    end_time = time.time()
    duration = end_time - start_time
    loggers.eval_logger.info(json.dumps({"duration_of_diff": f"{duration:.6f}", "jsx_nodes": len(G.get_all_jsx_component_nodes())}))
    start_time = time.time()
    comp_update_list = list(set(G.comp_update_queue))
    comp_update_list.sort()
    comp_update_list.reverse()
    loggers.eval_logger.info(json.dumps({"updating_times": len(comp_update_list)}))
    ...
    end_time = time.time()
    duration = end_time - start_time
    loggers.eval_logger.info(json.dumps({"duration_of_updating": f"{duration:.6f}"}))
...
def setup_graph_env(G: Graph):
    ...
    G.check_proto_pollution = options.prototype_pollution or options.vul_type == "proto_pollution"
...
def babel_convert():
    ...
    for now, we use system commands, ignoring node_modules and .git
    """
    try:
        shutil.rmtree(options.run_env)
    except:
    ...
    if os.path.isdir(options.babel):
        for item in os.listdir(options.babel):
            if item in ["node_modules", ".git", ".vscode", ".pnp.loader.mjs", ".pnp.cjs"]:
                continue
