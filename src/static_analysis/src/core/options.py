...
def parse_args():
    # Parse arguments
    parser = argparse.ArgumentParser(description="Object graph generator for JavaScript.")
    parser.add_argument("-p", "--print", action="store_true", help="Print logs to console, instead of file.")
    parser.add_argument("-t", "--vul-type", default="xss", help="Set the vulnerability type to be checked.")
    parser.add_argument("-P", "--prototype-pollution", "--pp", action="store_true", help="Check prototype pollution.")
    parser.add_argument(
        "-m",
        "--module",
        action="store_true",
        help="Module mode. Regard the input file as a module " "required by some other modules.",
    )
    parser.add_argument("-q", "--exit", action="store_true", default=False, help="Exit the program when vulnerability is found.")
    parser.add_argument(
        "-s",
        "--single-branch",
        action="store_true",
        help="Single branch. Do not create multiple " "possibilities when meet a branching point.",
    )
    parser.add_argument(
        "-a",
        "--run-all",
        action="store_true",
        default=False,
        help="Run all exported functions" "By default, only main functions will be run.",
    )
    parser.add_argument(
        "-f",
        "--function-timeout",
        type=float,
        help="Time limit when running all exported function, " "in seconds. (Defaults to no limit.)",
    )
    ...
    parser.add_argument("-c", "--call-limit", default=3, type=int, help="Set the limit of a call statement. " "(Defaults to 3.)")
    parser.add_argument("-e", "--entry-func")
    parser.add_argument("-l", "--list", action="store")

    parser.add_argument("--diff-list", type=str, help="")
    parser.add_argument("--repair-diff-path", type=str, help="")
    parser.add_argument("--cve-id", type=str, help="")
    parser.add_argument("--use-diff-files-as-entrance", action="store_true", default=False, help="")

    parser.add_argument("--avr-app-path", type=str, help="")
    parser.add_argument("--vul-graph-info-file-path", type=str, help="")
    parser.add_argument("--validation-output-path", type=str, help="")
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="If set, OPGen will limit of the max time of calls of each function in the call stack to max-rep",
    )
    parser.add_argument("--run-all-files", action="store_true", default=False, help="If set, OPGen will run all files of a package")
    parser.add_argument(
        "--no-prioritized-funcs",
        action="store_true",
        default=False,
        help="If set, OPGen will not try to run prioritized functions before everything",
    )
    ...
    parser.add_argument(
        "--add-sinks",
        type=str,
        help="If set, OPGen will treat the added function names as sink functions, separated by ,",
    )
    parser.add_argument(
        "--print-all-pathes",
        action="store_true",
        default=False,
        help="If set, OPGen will print all pathes even if the path is not a valid expolitable path",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Auto change the type of wildcard obj based on the called method",
    )
    ...
    parser.add_argument("--allow-spread", action="store_true", help="When set, enable spread attributes. Default is disabled")
    ...
    parser.add_argument("--allow-url-search-params", action="store_true", help="Model URLSearchParams")
    parser.add_argument("--network-response-source", action="store_true", help="Set network response as a source")
    parser.add_argument("--allow-img-src", action="store_true", help="Set img src as a sink")
    parser.add_argument("--allow-events", action="store_true", help="Analyze events")
    parser.add_argument("--gc", action="store_true", default=False, help="run a garbage collection after every function run")
    parser.add_argument(
        "--disable-prop-df",
        action="store_true",
        default=False,
        help="When set, no object-level data flow is set between an object an its property",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Run the pre-defined tests to make sure the installation finished",
    )
    parser.add_argument(
        "--validate",
        action="store_true",
        default=False,
        help="Validate Repairs",
    )
    parser.add_argument(
        "--slice",
        action="store_true",
