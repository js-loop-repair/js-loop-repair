...
class TraceRule:
    ...
    def end_with_jsx_attribute(self, attr_names, path):
        ...
        if end_node_attr.get("name") == "DangerouslySetHtmlContent":
            return True
        for e in self.graph.get_out_edges(end_node, edge_type=JSX_Edges.JSX_DATA_FLOW):
            ...
            if comp_node_attr.get("labels:label") == JSX_Nodes.JSX_DOM and (
                comp_node_attr.get("name") == "script" or comp_node_attr.get("name") == "style"
            ):
                return False
    ...
    def start_within_file(self, file_names, path):
        ...
        file_name = self.graph.get_ast_node_file_path(start_node)
    ...
    def jsx_has_spread_attrs(self, _, path):
        ...
        if options.allow_wildcard:
            first_node_passed = (
                first_node_attr.get("value") == wildcard
                and first_node_attr.get("labels:label") == JSX_Nodes.JSX_Component
            )
        if not first_node_passed:
            return False
        try:
            ...
            for ast_child in ast_children:
                ...
                if ast_type == "AST_JSXChildren" and not options.allow_spread:
                    return False
    ...
    def jsx_has_dom_xss_pair(self, _, path):
        ...
        for dom_tag, dom_attr in DOM_TAG_AND_ATTR_PAIRS:
            if (
                first_node_attr.get("labels:label") == "JSX_DOM"
                and first_node_attr.get("name") == dom_tag
                and second_node_attr.get("labels:label") == "JSX_Attribute"
                and second_node_attr.get("name") == dom_attr
            ):
                return True
        return False
    ...
    def jsx_has_no_indirect_src(self, _, path):
        ...
        for e in self.graph.get_in_edges(obj_node, edge_type="CONTRIBUTES_TO"):
            ...
            if (
                src_node_attr.get("labels:label") == "Object"
                and src_node_attr.get("type") == "string"
                and src_node_attr.get("code")
            ):
                return False
    ...
    def jsx_has_callee_of_dom_op(self, _, path):
        ...
        for node in path:
            node_attr = self.graph.get_node_attr(node)
            if (
                JSX_Label_Key in node_attr
                or node_attr.get("labels:label") == JSX_Nodes.JSX_DOM
                or node_attr.get("labels:label") == JSX_Nodes.JSX_Component
            ):
                return False
        for node in path:
            node_attr = self.graph.get_node_attr(node)
