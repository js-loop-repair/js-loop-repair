...
class NodeHandleResult:
    ...
    def __init__(self, **kwargs):
        ...
        self.jsx_nodes = kwargs.get("jsx_nodes", [])
        if isinstance(self.obj_nodes, set):
            self.obj_nodes = list(self.obj_nodes)
    ...
    def __bool__(self):
        return bool(self.obj_nodes or self.values or (self.name is not None) or self.name_nodes or self.used_objs)

    def __repr__(self):
        s = []
        for key in dir(self):
            if not key.startswith("_") and not callable(getattr(self, key)) and getattr(self, key):
...
class BranchTag:
    ...
    def __init__(self, s=None, **kwargs):
        ...
        if s:
            try:
                self.point, self.branch, self.mark = re.match(r"-?([^#]*)#(\d*)(\w?)", str(s)).groups()
                if self.point == "":
                    self.point = None
                if self.branch == "":
                    self.branch = None
...
class BranchTagContainer(list):
    ...
    def get_choice_tags(self):
        ...
        """
        return BranchTagContainer(filter(lambda i: i.point.startswith("If") or i.point.startswith("Switch"), self))
    ...
    def get_creating_for_tags(self):
        ...
        """
        return BranchTagContainer(filter(lambda i: i.point.startswith("For") and i.mark == "C", self))
    ...
    def match(self, tag: BranchTag = None, point=None, branch=None, mark=None) -> Tuple[int, BranchTag]:
        """
...
class ExtraInfo:
    def __init__(self, original=None, **kwargs):
        ...
        if original is not None:
            ...
            self.parent_component = original.parent_component
            self.scope = original.scope
            self.cur_obj = original.cur_obj
        ...
        if "cur_obj" in kwargs:
            self.cur_obj = kwargs.get("cur_obj")
        ...
        if "parent_component" in kwargs:
            self.parent_component = kwargs.get("parent_component")

    def __bool__(self):
        return bool(self.branches or (self.side is not None) or (self.parent_obj is not None) or (self.caller_ast is not None) or (self.switch_var is not None))
