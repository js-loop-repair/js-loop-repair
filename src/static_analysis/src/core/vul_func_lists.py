...
class Sinks:
    __signature_lists = {
        ...
        "sanitation": ["parseInt", "purify.sanitize"],
        "path_traversal": [
            "pipe",
            "sink_hqbpillvul_http_write",
            "sink_hqbpillvul_http_sendFile",
        ],
        "depd": ["sink_hqbpillvul_pp", "sink_hqbpillvul_code_execution", "sink_hqbpillvul_exec"],
    }
    ...
    def get_all_sinks(self, add_sinks=True):
        all_sinks = set()
        for vul_type in self.__signature_lists:
            sinks = self.get_sinks_by_vul_type(vul_type, add_sinks=add_sinks)
