    ...
    */
    ...
    switch (currentNode.type) {
                        ...
                        if (param.type == 'Identifier') { // no default value
                            ...
                            for (let i = 0; i < param.properties.length; i++) {
                                ...
                                if (property.type === 'RestElement') {
                                    ...
                                    throw new Error(`Unsupported Object Pattern property type: ${property.type}`)
                                }

                                nodeIdCounter++;
                                relsStream.push([vParameterId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                                const extra = { parentType: "ObjectPattern" };
                                dfs(property, nodeIdCounter, vParameterId, i, currentFunctionId, extra);
                            }
                        }
        ...
        case 'RestElement':
            ...
            break;
        case 'ObjectPattern':
            console.log(`  Warning: uncompleted support for ${currentNode.type}`, currentNode, extra);
        ...
        case 'Property':
            if (outputStyle == 'php') {
                if (extra && extra.parentType === 'ObjectPattern') {
                    if (currentNode.value.type === 'Identifier') {
                        nodeIdCounter++;
                        relsStream.push([currentId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                        dfs(currentNode.value, nodeIdCounter, currentId, 0, currentFunctionId, null);
                    } else if (currentNode.value.type === 'ObjectPattern') {
                        nodeIdCounter++;
                        relsStream.push([currentId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                        dfs(currentNode.value, nodeIdCounter, currentId, 0, currentFunctionId, null);
                    } else if (currentNode.value.type === 'AssignmentPattern') {
                        nodeIdCounter++;
                        relsStream.push([currentId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                        dfs(currentNode.value, nodeIdCounter, currentId, 0, currentFunctionId, null);
                    }
                    ...
                    nodes[currentId] = {
                        label: 'AST',
                        type: currentNode.type,
                        phptype: 'AST_ObjectPattern_ELEM',
                        lineLocStart: currentNode.loc ? currentNode.loc.start.line : null,
                        childNum: childNum,
                        lineLocEnd: currentNode.loc ? currentNode.loc.end.line : null,
                        colLocStart: currentNode.loc ? currentNode.loc.start.column : null,
        ...
        case 'Super':
            ...
            break;
        case 'JSXElement':
            // Always set the opening element's childNum as 0. This allows us to easily get the element's identifier by indexing.
            if (currentNode.openingElement) {
                nodeIdCounter++;
                relsStream.push([currentId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                dfs(currentNode.openingElement, nodeIdCounter, currentId, childNumberCounter, currentFunctionId, null);
            }
            if (currentNode.children && currentNode.children.length > 0) {
                ...
                const JSXChildrenId = nodeIdCounter;
                for (const child of currentNode.children) {
                    nodeIdCounter++;
                    childNumberCounter++;
                    relsStream.push([JSXChildrenId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                    dfs(child, nodeIdCounter, JSXChildrenId, childNumberCounter, currentFunctionId, null);
                }
            }
            nodes[currentId] = {
                label: 'AST',
                phptype: "AST_JSXElement",
                lineLocStart: currentNode.loc ? currentNode.loc.start.line : null,
                lineLocEnd: currentNode.loc ? currentNode.loc.end.line : null,
                colLocStart: currentNode.loc ? currentNode.loc.start.column : null,
                colLocEnd: currentNode.loc ? currentNode.loc.end.column : null,
        ...
        case 'JSXOpeningElement':
            if (currentNode.name) {
                // Always set the identifier's childNum as 0. This allows us to easily get the element's identifier by indexing.
                nodeIdCounter++;
                relsStream.push([currentId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                dfs(currentNode.name, nodeIdCounter, currentId, childNumberCounter, currentFunctionId, null);
            }
            if (currentNode.attributes && currentNode.attributes.length > 0) {
                ...
                const JSXAttributesId = nodeIdCounter;
                for (const attr of currentNode.attributes) {
                    nodeIdCounter++;
                    childNumberCounter++;
                    relsStream.push([JSXAttributesId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                    dfs(attr, nodeIdCounter, JSXAttributesId, childNumberCounter, currentFunctionId, null);
                }
            }
            nodes[currentId] = {
                label: 'AST',
                phptype: "AST_JSXOpeningElement",
                lineLocStart: currentNode.loc ? currentNode.loc.start.line : null,
                lineLocEnd: currentNode.loc ? currentNode.loc.end.line : null,
                colLocStart: currentNode.loc ? currentNode.loc.start.column : null,
                colLocEnd: currentNode.loc ? currentNode.loc.end.column : null,
        ...
        case 'JSXAttribute':
            if (currentNode.name) {
                ...
                nodes[nodeIdCounter] = {
                    label: 'AST',
                    phptype: "AST_JSXAttribute_Name",
                    lineLocStart: currentNode.name.loc ? currentNode.name.loc.start.line : null,
                    lineLocEnd: currentNode.name.loc ? currentNode.name.loc.end.line : null,
                    colLocStart: currentNode.name.loc ? currentNode.name.loc.start.column : null,
                    colLocEnd: currentNode.name.loc ? currentNode.name.loc.end.column : null,
                    childNum: childNumberCounter,
                    funcId: currentFunctionId,
                    comment: comment,
                    loc: currentNode.loc,
                    code: getCode(currentNode.name, sourceCode)
                };
            }

            if (currentNode.value || currentNode.value === null) {
                const loc = currentNode.value ? currentNode.value.loc : currentNode.name.loc
                nodeIdCounter++;
                childNumberCounter++;
                relsStream.push([currentId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                nodes[nodeIdCounter] = {
                    label: 'AST',
                    phptype: "AST_JSXAttribute_Value",
                    lineLocStart: loc ? loc.start.line : null,
                    lineLocEnd: loc ? loc.end.line : null,
                    colLocStart: loc ? loc.start.column : null,
                    colLocEnd: loc ? loc.end.column : null,
                    childNum: childNumberCounter,
                    funcId: currentFunctionId,
                    comment: comment,
                    loc: loc,
                    code: getCode(currentNode.value || currentNode.name, sourceCode)
                };
                const JSXAttributeValueId = nodeIdCounter;

                const explicitLiteralTrueExpression = {
                    type: "JSXExpressionContainer",
                    start: currentNode.name.start,
                    end: currentNode.name.end,
                    loc: currentNode.name.loc,
                    range: currentNode.name.range,
                    expression: {
                        type: "Literal",
                        start: currentNode.name.start,
                        end: currentNode.name.end,
                ...
                const childValue = currentNode.value
                    ? currentNode.value
                    : explicitLiteralTrueExpression;

                nodeIdCounter++;
                relsStream.push([JSXAttributeValueId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                dfs(childValue, nodeIdCounter, JSXAttributeValueId, childNumberCounter, currentFunctionId, null);
            }

            nodes[currentId] = {
                label: 'AST',
                phptype: "AST_JSXAttribute",
                lineLocStart: currentNode.loc ? currentNode.loc.start.line : null,
                lineLocEnd: currentNode.loc ? currentNode.loc.end.line : null,
                colLocStart: currentNode.loc ? currentNode.loc.start.column : null,
                colLocEnd: currentNode.loc ? currentNode.loc.end.column : null,
            ...
            break;
        case 'JSXText':
            // if the JSX Text is empty, add a NULL
            const text = getCode(currentNode, sourceCode);
            if (!text || /^[\s]*$/.test(text)) {
                nodes[nodeIdCounter] = {
                    label: 'AST',
                    type: 'NULL',
        ...
        case 'JSXSpreadAttribute': {
            // TODO: support JSX spread attribute
            nodeIdCounter++;
            relsStream.push([currentId, nodeIdCounter, parentOf].join(delimiter) + '\n');
            nodes[nodeIdCounter] = {
                label: 'AST',
                ...
                comment: comment,
                loc: currentNode.loc,
                code: 'JSXSpreadAttribute'
            };

            const currentNodeValue = currentNode.argument
            if (currentNodeValue || currentNodeValue === null) {
                const loc = currentNodeValue ? currentNodeValue.loc : currentNode.name.loc
                nodeIdCounter++;
                childNumberCounter++;
                relsStream.push([currentId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                nodes[nodeIdCounter] = {
                    ...
                    comment: comment,
                    loc: loc,
                    code: getCode(currentNodeValue, sourceCode)
                };
                const JSXAttributeValueId = nodeIdCounter;
                const childValue = currentNodeValue;
                nodeIdCounter++;
                relsStream.push([JSXAttributeValueId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                dfs(childValue, nodeIdCounter, JSXAttributeValueId, childNumberCounter, currentFunctionId, null);
            }
        ...
        case 'JSXExpressionContainer':
            if (currentNode.expression) {
                nodeIdCounter++;
                relsStream.push([currentId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                dfs(currentNode.expression, nodeIdCounter, currentId, childNumberCounter, currentFunctionId, null);
            }
            nodes[currentId] = {
                label: 'AST',
                phptype: "AST_JSXExpressionContainer",
                lineLocStart: currentNode.loc ? currentNode.loc.start.line : null,
                lineLocEnd: currentNode.loc ? currentNode.loc.end.line : null,
                colLocStart: currentNode.loc ? currentNode.loc.start.column : null,
                colLocEnd: currentNode.loc ? currentNode.loc.end.column : null,
        ...
        case 'JSXIdentifier':
            if (outputStyle == 'php') {
                // TODO: Maybe we can set the "phpfalg" attribute to indicate whether this element is a DOM element or a React.js Component. But we can also check this in the abstract interpretation.
                nodes[currentId] = {
                    label: 'AST',
                    phptype: "AST_JSXIdentifier",
                    lineLocStart: currentNode.loc ? currentNode.loc.start.line : null,
                    lineLocEnd: currentNode.loc ? currentNode.loc.end.line : null,
                    colLocStart: currentNode.loc ? currentNode.loc.start.column : null,
                    colLocEnd: currentNode.loc ? currentNode.loc.end.column : null,
        ...
        case 'ImportDeclaration':
            if (outputStyle == 'php') {
                ...
                if (sourceType === 'Literal') {
                    ...
                    throw new Error(`No source value for ${currentNode}`);
                }

                const moduleName = currentNode.source.value;
                console.log('search module', moduleName, filename)
                const modulePath = searchModule(moduleName, filename)[0];
                requiredModules.add(modulePath);
                nodeIdCounter++;
                childNumberCounter++;
                relsStream.push([currentId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                nodes[nodeIdCounter] = {
                    label: 'AST',
                    phptype: "AST_ImportDeclaration_Source",
                    lineLocStart: currentNode.source.loc ? currentNode.source.loc.start.line : null,
                    lineLocEnd: currentNode.source.loc ? currentNode.source.loc.end.line : null,
                    colLocStart: currentNode.source.loc ? currentNode.source.loc.start.column : null,
                    colLocEnd: currentNode.source.loc ? currentNode.source.loc.end.column : null,
                    childNum: childNumberCounter,
                    funcId: currentFunctionId,
                    comment: comment,
                    loc: currentNode.source.loc,
                    code: sourceValue,
                    name: modulePath
                };

                const specifiers = currentNode.specifiers;
                const supporttedSpecifierType = new Set(['ImportDefaultSpecifier', 'ImportSpecifier']);
                const unhandledSpecifierType = new Set(['ImportNamespaceSpecifier']);
                for (const specifier of specifiers) {
                    const specifierType = specifier.type;
                    if (!supporttedSpecifierType.has(specifierType) && !unhandledSpecifierType.has(specifierType)) {
                        throw new Error(`Does not support specifierType ${specifierType}`)
                    }
                    nodeIdCounter++;
                    childNumberCounter++;
                    relsStream.push([currentId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                    nodes[nodeIdCounter] = {
                        label: 'AST',
                        phptype: `AST_${specifierType}`,
                        lineLocStart: specifier.loc ? specifier.loc.start.line : null,
                        lineLocEnd: specifier.loc ? specifier.loc.end.line : null,
                        colLocStart: specifier.loc ? specifier.loc.start.column : null,
                        colLocEnd: specifier.loc ? specifier.loc.end.column : null,
                        childNum: childNumberCounter,
                        funcId: currentFunctionId,
                        comment: comment,
                        loc: specifier.loc,
                    ...
                    const specifierNodeId = nodeIdCounter;
                    const importedSpecifierName = specifier.imported ? specifier.imported.name : undefined;
                    const localSpecifierName = specifier.local.name;

                    nodeIdCounter++;
                    relsStream.push([specifierNodeId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                    nodes[nodeIdCounter] = {
                        label: 'AST',
                        phptype: `AST_${specifierType}_Imported_Identifier`,
                        lineLocStart: specifier.loc ? specifier.loc.start.line : null,
                        lineLocEnd: specifier.loc ? specifier.loc.end.line : null,
                        colLocStart: specifier.loc ? specifier.loc.start.column : null,
                        colLocEnd: specifier.loc ? specifier.loc.end.column : null,
                        ...
                        loc: specifier.loc,
                        code: getCode(currentNode, sourceCode),
                        name: importedSpecifierName || localSpecifierName
                    };
                    ...
                    nodes[nodeIdCounter] = {
                        label: 'AST',
                        phptype: `AST_${specifierType}_Local_Identifier`,
                        lineLocStart: specifier.loc ? specifier.loc.start.line : null,
                        lineLocEnd: specifier.loc ? specifier.loc.end.line : null,
                        colLocStart: specifier.loc ? specifier.loc.start.column : null,
                        colLocEnd: specifier.loc ? specifier.loc.end.column : null,
                        ...
                        loc: specifier.loc,
                        code: getCode(currentNode, sourceCode),
                        name: localSpecifierName
                    };
                }

                nodes[currentId] = {
                    label: 'AST',
                    phptype: "AST_ImportDeclaration",
                    lineLocStart: currentNode.loc ? currentNode.loc.start.line : null,
                    lineLocEnd: currentNode.loc ? currentNode.loc.end.line : null,
                    colLocStart: currentNode.loc ? currentNode.loc.start.column : null,
                    colLocEnd: currentNode.loc ? currentNode.loc.end.column : null,
        ...
        case 'ExportNamedDeclaration':
            ...
            for (const specifier of specifiers) {
                ...
                if (!local || !exported) {
                    throw new Error(
                        `Couldn't find exported local or exported name`
                    );
                }
                nodeIdCounter++;
                childNumberCounter++;
                ...
                nodes[nodeIdCounter] = {
                    label: 'AST',
                    phptype: `AST_ExportNamedDeclaration_Specifier`,
                    lineLocStart: specifier.loc ? specifier.loc.start.line : null,
                    lineLocEnd: specifier.loc ? specifier.loc.end.line : null,
                    colLocStart: specifier.loc ? specifier.loc.start.column : null,
                    colLocEnd: specifier.loc ? specifier.loc.end.column : null,
                ...
                const exported_specifier_nodeId = nodeIdCounter;
                nodeIdCounter++;
                relsStream.push([exported_specifier_nodeId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                dfs(local, nodeIdCounter, exported_specifier_nodeId, 0, currentFunctionId, null);

                nodeIdCounter++;
                relsStream.push([exported_specifier_nodeId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                dfs(exported, nodeIdCounter, exported_specifier_nodeId, 1, currentFunctionId, null);
            }
            if (currentNode.declaration && currentNode.specifiers.length === 0) {
                nodeIdCounter++;
                childNumberCounter++;
                relsStream.push([currentId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                dfs(currentNode.declaration, nodeIdCounter, currentId, childNumberCounter, currentFunctionId, { childNumberCounter: childNumberCounter });
            }
            nodes[currentId] = {
                label: 'AST',
                phptype: "AST_ExportNamedDeclaration",
                lineLocStart: currentNode.loc ? currentNode.loc.start.line : null,
                lineLocEnd: currentNode.loc ? currentNode.loc.end.line : null,
                colLocStart: currentNode.loc ? currentNode.loc.start.column : null,
                colLocEnd: currentNode.loc ? currentNode.loc.end.column : null,
        ...
        case 'ExportDefaultDeclaration':
            if (outputStyle == 'php') {
                if (currentNode.declaration) {
                    nodeIdCounter++;
                    relsStream.push([currentId, nodeIdCounter, parentOf].join(delimiter) + '\n');
                    dfs(currentNode.declaration, nodeIdCounter, currentId, 0, currentFunctionId, null);
                }
                const identifier = currentNode.declaration.name;
                nodes[currentId] = {
                    label: 'AST',
                    phptype: "AST_ExportDefaultDeclaration",
                    phpflag: identifier,
                    lineLocStart: currentNode.loc ? currentNode.loc.start.line : null,
                    lineLocEnd: currentNode.loc ? currentNode.loc.end.line : null,
                    colLocStart: currentNode.loc ? currentNode.loc.start.column : null,
        ...
        case 'ExportAllDeclaration':
            if (outputStyle == 'php') {
                ...
                nodes[currentId] = {
                    label: 'AST',
                    phptype: "AST_ExportAllDeclaration",
                    lineLocStart: currentNode.loc ? currentNode.loc.start.line : null,
                    lineLocEnd: currentNode.loc ? currentNode.loc.end.line : null,
                    colLocStart: currentNode.loc ? currentNode.loc.start.column : null,
                    colLocEnd: currentNode.loc ? currentNode.loc.end.column : null,
        ...
        case 'JSXMemberExpression':
            if (outputStyle == 'php') {
                nodes[currentId] = {
                    label: 'AST',
                    phptype: "AST_JSXMemberExpression",
                    lineLocStart: currentNode.loc ? currentNode.loc.start.line : null,
                    lineLocEnd: currentNode.loc ? currentNode.loc.end.line : null,
                    colLocStart: currentNode.loc ? currentNode.loc.start.column : null,
                    colLocEnd: currentNode.loc ? currentNode.loc.end.column : null,
        ...
        case 'JSXEmptyExpression':
            nodes[currentId] = {
                label: 'AST',
                phptype: "AST_JSXEmptyExpression",
                lineLocStart: currentNode.loc ? currentNode.loc.start.line : null,
                lineLocEnd: currentNode.loc ? currentNode.loc.end.line : null,
                colLocStart: currentNode.loc ? currentNode.loc.start.column : null,
                colLocEnd: currentNode.loc ? currentNode.loc.end.column : null,
