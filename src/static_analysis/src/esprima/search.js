...
function searchModule(moduleName, requiredBy) {
    var selfBuiltPackages = ['yargs', 'execa', 'send', 'async',
    ...
    if (moduleName.indexOf("@/") == 0) {
        currentSearchPath = requiredBy;
        while (currentSearchPath != path.resolve(currentSearchPath, "..")) {
            searchPaths.add(currentSearchPath);
            currentSearchPath = path.resolve(currentSearchPath, "..");
        }
    }

    // Do not search in node_modules
    ...
    for (let p of searchPaths) {
        ...
        if (!found) {
            let jsxPath = path.resolve(p, moduleName);
            if (!moduleName.endsWith('.jsx')) {
                jsxPath += '.jsx'
            }
            if (fs.existsSync(jsxPath) && fs.statSync(jsxPath).isFile()) {
                console.error(`Package ${moduleName} found at ${jsxPath}`.white.inverse);
                found = true;
                modulePath = jsxPath;
                mainPath = jsxPath;
                break;
        ...
        if (!found) {
            if (moduleName.indexOf('@/') == 0) {
                realModuleName = moduleName.replace('@/', './')
                let jsxPath = path.resolve(p, realModuleName);
                let jsPath = jsxPath;
                if (!moduleName.endsWith('.js')) {
                    jsPath += '.js'
                }
