...
class HandleArray(Handler):
    ...
    def process(self):
        ...
        for child in children:
            result = self.internal_manager.dispatch_node(child, ExtraInfo(self.extra, parent_obj=added_obj))
            # used_objs.update(result.obj_nodes)

        G.add_obj_as_prop(
            prop_name="length", js_type="number", value=len(children), ast_node=node_id, parent_obj=added_obj
        )

        return NodeHandleResult(obj_nodes=[added_obj], used_objs=list(used_objs), callback=get_df_callback(G))
    ...
    def process(self):
        if not (self.extra and self.extra.parent_obj is not None):
            ...
            for obj in value_objs:
                self.G.add_obj_as_prop(key, self.node_id, parent_obj=self.extra.parent_obj, tobe_added_obj=obj)
        return NodeHandleResult(obj_nodes=value_objs, callback=get_df_callback(self.G))  # used_objs=used_objs,
    ...
    def process(self):
        ...
        for v in values:
            if v == wildcard or v is None:
                ...
                continue
            v = val_to_float(v)
            if v != float("nan") and G.get_node_attr(node_id).get("flags:string[]") == "UNARY_MINUS":
                new_values.append(-v)
            else:
                new_values.append(v)
