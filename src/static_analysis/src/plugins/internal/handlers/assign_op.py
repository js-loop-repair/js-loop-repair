...
def handle_op_by_objs(G, ast_node, extra=ExtraInfo(), saved={}, manager=None):
    ...
    used_objs = set()
    if len(left_objs) * len(right_objs) > 10:
    ...
    if G.get_node_attr(ast_node).get("type") == "AST_ASSIGN_OP":
        operators.do_assign(G, handled_left, NodeHandleResult(obj_nodes=results), branches=extra.branches, ast_node=ast_node)
    res = NodeHandleResult(ast_node=ast_node, obj_nodes=results, used_objs=list(used_objs), callback=get_df_callback(G))
    if len(results) > 10:
