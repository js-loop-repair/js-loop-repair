...
def simurun_block(G, ast_node, parent_scope=None, branches=None, block_scope=True, decl_var=False):
    """
    ...
    if block_scope:
        G.cur_scope = G.add_scope("BLOCK_SCOPE", decl_ast=ast_node, scope_name=G.scope_counter.gets(f"Block{ast_node}"))
    ...
    loggers.main_logger.log(ATTENTION, "stmts: {}".format(stmts))
    # simulate statements
    for stmt in stmts:
...
def JSX_simurun_block(G: Graph, ast_node, parent_scope=None, branches=None, block_scope=True, decl_var=False):
    """
    Simurun a block by running its statements one by one.
    A block is a BlockStatement in JavaScript,
    or an AST_STMT_LIST in PHP.
    ...
    if block_scope:
        G.cur_scope = G.add_scope("BLOCK_SCOPE", decl_ast=ast_node, scope_name=G.scope_counter.gets(f"Block{ast_node}"))
    loggers.main_logger.log(ATTENTION, f"JSX BLOCK STARTS {ast_node} , SCOPE {G.cur_scope}")

    decl_vars_and_funcs(G, ast_node, var=decl_var)
    stmts = G.get_ordered_ast_child_nodes(ast_node)

    # simulate statements
    loggers.main_logger.log(ATTENTION, f"JSX SIMU BLOCK  {ast_node} stmts: {stmts}")
    for stmt in stmts:
        if G.cfg_stmt is not None:
            G.add_edge_if_not_exist(G.cfg_stmt, stmt, {"type:TYPE": "FLOWS_TO"})
    ...
    returned_objs = G.function_returns[G.find_ancestor_scope()][1]
    loggers.main_logger.log(ATTENTION, "JSX_simurun_block returned_objs: {}".format(returned_objs))
