...
def handle_class(G: Graph, ast_node, extra):
    loggers.code_slice_logger.info(f"handle class {ast_node}")
    children = G.get_ordered_ast_child_nodes(ast_node)
    loggers.code_slice_logger.info(f"children:  {children}")
    ...
    if len(children) > 4:
        ...
        loggers.code_slice_logger.warning("couldnt find class body")
        return NodeHandleResult(obj_nodes=[class_obj])
