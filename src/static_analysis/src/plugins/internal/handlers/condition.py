        ...
        for if_elem in if_elems:
            ...
            if G.get_node_attr(condition).get("type") == "NULL":  # else
                if else_is_deterministic or G.single_branch:
                    ...
                    branch_tag = BranchTag(point=stmt_id, branch=str(branch_num_counter))
            ...
            loggers.main_logger.debug(
                "Check condition {} result: possibility: {} deterministic: {}".format(
                    sty.ef.i + G.get_node_attr(condition).get("code") + sty.rs.all, possibility, deterministic
                )
            )
...
class HandleConditional(Handler):
    def process(self):
        ...
        loggers.main_logger.debug(
            f'Condition: {G.get_node_attr(node_id).get("code")} deterministic: {deterministic}, possibility: {possibility}'
        )
        if deterministic and possibility == 1:
