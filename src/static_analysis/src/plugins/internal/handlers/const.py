...
class HandleConst(Handler):
    ...
    def process(self):
        ...
        if cur_type == "integer" and code.startswith("0x") or code.startswith("0X"):
            ...
            value = int(code, 2)
        elif cur_type == "integer" and (code.startswith("0o") or code.startswith("0O")):
            value = int(code, 8)
        elif cur_type == "string":
            if self.G.get_node_attr(self.node_id).get("flags:string[]") == "JS_REGEXP":
                added_obj = G.add_obj_node(ast_node=self.node_id, js_type=None, value=code)
                G.add_obj_as_prop("__proto__", parent_obj=added_obj, tobe_added_obj=self.G.regexp_prototype)
