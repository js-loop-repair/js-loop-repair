...
class HandleEncapsList(Handler):
    ...
    def process(self):
        return handle_template(self.G, self.node_id, self.extra)
...
def handle_template(G: Graph, ast_node, extra=ExtraInfo()):
    ...
    if len(children) == 0:
        return NodeHandleResult(ast_node=ast_node, values=[""], value_sources=[ast_node])
    if len(children) == 1:
    ...
    return NodeHandleResult(
        ast_node=ast_node, obj_nodes=results, used_objs=list(all_used_objs), callback=get_df_callback(G)
    )
