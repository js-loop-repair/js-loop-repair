    ...
    for exported_obj in exported_obj_nodes:
        ...
        if cur_file_stub_objs:
            for stub_obj in stub_objs:
                ...
                if (
                    "ES6_stub_specifier_name" in stub_obj_attr
                    and stub_obj_attr["ES6_stub_specifier_name"] == "ES6_stub_export_default"
                ):
...
def add_exported_obj_to_file_scope(G: Graph, name: str, exported_obj):
    ...
    added_export_default_node = G.add_obj_to_scope(
        name=name, ast_node=file_scope_ast, scope=file_scope, tobe_added_obj=exported_obj
    )

    loggers.main_logger.info(
        f"Added exported_obj {exported_obj} to file_scope {file_scope}, name: {name}, added_node = {added_export_default_node}"
    )
