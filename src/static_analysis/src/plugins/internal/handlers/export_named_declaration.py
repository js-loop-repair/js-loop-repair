...
def ast_export_named_declaration(G: Graph, ast_node, extra):
    ...
    if source_file_path:
        handle_export_from_source(
            G=G,
            source_file_path=source_file_path,
            ast_node=ast_node,
            cur_file_path=cur_file_path,
            extra=extra,
            cur_file_stub_objs=cur_file_stub_objs,
        )
    else:
        ...
        if len(exported_declarations) == 1:
            ...
            if ast_child_type == "AST_ASSIGN" or ast_child_type == "AST_FUNC_DECL" or ast_child_type == "AST_CLASS":
                handle_export_declaration(
                    G=G, declaration_ast_node=ast_child, extra=extra, stub_objs=cur_file_stub_objs
                )
...
def handle_export_from_source(G: Graph, source_file_path: str, ast_node: str, cur_file_path, extra, cur_file_stub_objs):
    ...
    if not source_node:
        loggers.main_logger.warn(
            f"[Export Named Declaration] Couldn't find the source for {source_file_path}, AST_Node: {ast_node}"
        )
        return []
    ...
    for decl in exported_declarations:
        ...
        local_r = handle_var(
            ...
            ast_node=local,
            side="right",
            extra=ExtraInfo(original=extra, side="right", scope=G.find_ancestor_scope()),
        )
        exported_r = handle_var(
            G,
            ast_node=exported,
            side="right",
            extra=ExtraInfo(original=extra, side="right", scope=G.find_ancestor_scope()),
...
def handle_export_specifier(G: Graph, specifier_node, extra, stub_objs=None):
    ...
    r = handle_var(
        G, ast_node=local, side="right", extra=ExtraInfo(original=extra, side="right", scope=G.find_ancestor_scope())
    )
