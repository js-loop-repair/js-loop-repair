...
def run_toplevel_file(G: Graph, node_id):
    ...
    func_decl_obj = decl_function(
        G, node_id, func_name=file_path, obj_parent_scope=G.BASE_SCOPE, scope_parent_scope=G.BASE_SCOPE
    )
    ...
    # add module.exports
    added_module_exports = G.add_obj_as_prop("exports", node_id, parent_obj=added_module_obj)
...
def ES6_run_toplevel_file(G: Graph, node_id):
    ...
    G.module_registry[file_path] = {"state": "Loading"}
    previous_file_path = G.cur_file_path
    G.cur_file_path = file_path
    ...
    loggers.main_logger.info(
        sty.fg(173) + sty.ef.inverse + "RUNNING TOP_LEVEL_FILE BEGINS: {}".format(file_path) + sty.rs.all
    )

    # add function object and scope
    func_decl_obj = decl_function(
        G, node_id=node_id, func_name=file_path, obj_parent_scope=G.BASE_SCOPE, scope_parent_scope=G.BASE_SCOPE
    )
    func_scope = G.add_scope(
        scope_type="FILE_SCOPE",
        decl_ast=node_id,
    ...
    G.file_stack.pop(-1)
    G.module_registry[file_path] = {"state": "Loaded"}
    # loggers.main_logger.info("{} exported: ".format(file_path))
