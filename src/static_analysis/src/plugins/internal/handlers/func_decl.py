...
class HandleFuncDecl(Handler):
    ...
    def process(self):
        obj_nodes = self.G.get_func_decl_objs_by_ast_node(self.node_id, scope=self.G.find_ancestor_scope())
        if not obj_nodes:
            obj_nodes = [decl_function(self.G, self.node_id)]
        return NodeHandleResult(obj_nodes=obj_nodes)


def decl_function(G, node_id, func_name=None, obj_parent_scope=None, scope_parent_scope=None, add_to_scope=True):
    """
