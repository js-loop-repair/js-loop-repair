...
from .blocks import simurun_block, JSX_simurun_block
...
def ast_call_function(G, ast_node, extra):
    ...
    if G.get_node_attr(ast_node).get("flags:string[]") == "JS_TYPEOF":
        ...
        if handled_args:
            for obj in handled_args[0].obj_nodes:
                ...
                elif G.get_node_attr(obj).get("type") == "object" and G.get_node_attr(obj).get("code") == wildcard:
        ...
        return NodeHandleResult(values=returned_values, value_sources=returned_value_sources)
    ...
    func_decl_objs = list(filter(lambda x: x != G.undefined_obj and x != G.null_obj, handled_callee.obj_nodes))
    func_name = handled_callee.name
    # add blank functions
    if not func_decl_objs:
        if handled_callee.name_nodes:
            for name_node in handled_callee.name_nodes:
                func_decl_obj = G.add_blank_func_with_og_nodes(func_name or "{anonymous}")
...
def simurun_function(G, func_ast, branches=None, block_scope=True, caller_ast=None):
    """
    ...
    if caller_ast is not None:
        if G.call_counter[caller_ast] >= G.call_limit:
            loggers.main_logger.warning(f"{caller_ast}: Function {func_ast} in call stack " f"{G.call_counter[caller_ast]} times, skip simulating")
    ...
    loggers.main_logger.info(
        ...
        + "FUNCTION {} {} STARTS, SCOPE {}, DECL OBJ {}, this OBJs {}, branches {}".format(
            func_ast, func_name or "{anonymous}", G.cur_scope, func_obj, G.cur_objs, branches
        )
        + sty.rs.all
    )
    ...
    for child in G.get_child_nodes(func_ast, child_type="AST_STMT_LIST"):
        returned_objs, used_objs = simurun_block(G, child, parent_scope=G.cur_scope, branches=branches, block_scope=block_scope, decl_var=True)
        break
...
def ES6_simurun_function(G: Graph, func_ast, branches=None, block_scope=True, caller_ast=None):
    """
    Simurun a function by running its body.
    """
    loggers.main_logger.info(f"JSX simu run function {func_ast}, caller_ast {caller_ast}")
    if branches is None or G.single_branch:
        # create an instance of BranchTagContainer every time,
        # don't use default argument
        branches = BranchTagContainer()
    ...
    func_name = G.get_node_attr(func_obj).get("name") if func_objs else "?"
    loggers.main_logger.info(
        f'{sty.ef.inverse}{sty.fg.green}FUNCTION STARTS {func_ast} {func_name or "{anonymous}"} , SCOPE {G.cur_scope}, DECL OBJ {func_obj}, this OBJs {G.cur_objs}, branches {branches}{sty.rs.all}'
    )
    returned_objs, used_objs = [], []
    # update graph register for cur_func
    G.cur_func = G.get_cur_function_decl()
    ...
    for child in G.get_child_nodes(func_ast, child_type="AST_STMT_LIST"):
        returned_objs, used_objs = JSX_simurun_block(G, child, parent_scope=G.cur_scope, branches=branches, block_scope=block_scope, decl_var=True)
        break
    ...
    for node in toplevel_nodes:
        if G.get_node_attr(node).get("name") == file_path:
            ...
            if saved_module_exports != None:
                ...
                loggers.main_logger.log(
                    ATTENTION,
                    "File has been required, " "return saved module.exports {} for {}".format(module_exports_objs, file_path),
                )
                break
            else:
...
def handle_require(G: Graph, caller_ast, extra, _, module_names):
    from src.plugins.internal.modeled_js_builtins import setup_express, setup_mongoose, setup_sqlite
    ...
    returned_objs = set()
    if options.skip_require:
        return NodeHandleResult(obj_nodes=returned_objs, used_objs=list(chain(*src)))

    for module_name in module_names:
        if module_name == "express":
            module_objs = setup_express(G).obj_nodes
            returned_objs.update(setup_express(G).obj_nodes)
            continue
        elif module_name == "mongoose":
            returned_objs.update(setup_mongoose(G).obj_nodes)
            continue
        elif module_name == "sqlite":
            returned_objs.update(setup_sqlite(G).obj_nodes)
            continue

        file_path = G.get_node_attr(caller_ast).get("name")
        if file_path and options.require_diff_file and (file_path not in options.diff_file_list):
            loggers.code_slice_logger.info(f"SKIP require:  {file_path}")
            continue
        else:
            loggers.code_slice_logger.info(f"requiring:  {file_path}")

        if False and module_name in modeled_builtin_modules.modeled_modules and G.vul_type != "path_traversal":
            ...
            loggers.main_logger.info("Trying to require package {}.".format(module_name))
            if module_name is None or module_name == wildcard:
                logger.error("{} trying to require unknown package.".format(caller_ast))
                continue
            if not module_exports_objs:
                # check if the file's AST is in the graph
                file_path, _ = esprima_search(module_name, G.get_cur_file_path(), print_func=logger.info)
            ...
            if not module_exports_objs:
                ...
                logger.log(ATTENTION, f"Generating AST on demand for module " f"{module_name} at {file_path}...")

                # following code is copied from analyze_files,
                # consider combining in future.
                start_id = str(G.cur_id)
                result = esprima_parse(file_path, ["-n", start_id, "-o", "-"], print_func=logger.info)
                G.import_from_string(result)
                # start from the AST_TOPLEVEL node instead of the File node
                module_exports_objs = file.run_toplevel_file(G, str(int(start_id) + 1))
            ...
            if module_exports_objs:
                ...
                logger.error("Required module {} at {} not found!".format(module_name, file_path))
...
def instantiate_obj(G, exp_ast_node, constructor_decl, branches=None):
    ...
    returned_objs, _ = simurun_function(G, constructor_decl, branches=branches, caller_ast=exp_ast_node)
...
def run_exported_functions(G: Graph, module_exports_objs, extra, mark_fake_args=True):
    loggers.main_logger.info(f"running functions {module_exports_objs}")
    ...
    while len(exported_objs) != 0:  # BFS
        ...
        loggers.main_logger.info(f"cur_func_ast:  {cur_func_ast} {obj}")
        ...
        if G.func_entry_point is not None and not (
            G.get_node_attr(obj).get("type") == "function" and (G.get_node_attr(obj).get("name") == G.func_entry_point or cur_obj_name == G.func_entry_point)
        ):
            continue
        ...
        loggers.main_logger.info(f"obj:  {obj} {G.get_node_attr(obj)}")
            ...
            if options.exported_func_timeout is not None:
                try:
                    returned_result, newed_objs = func_timeout(
                        ...
                        call_function,
                        args=(G, [obj], [], NodeHandleResult, extra, None, True, "Unknown", None, True, None),
                    )
                except FunctionTimedOut as err:
                    print(err)
            else:
                returned_result, newed_objs = call_function(G, [obj], extra=extra, is_new=True, mark_fake_args=mark_fake_args)
            ...
            if options.exported_func_timeout is not None:
                try:
                    with timeout(seconds=options.exported_func_timeout, error_message="exported func timeout"):
                        returned_result, newed_objs = call_function(
                            G,
                            [obj],
                ...
                returned_result, newed_objs = call_function(G, [obj], this=NodeHandleResult(obj_nodes=[parent_obj]), extra=extra, mark_fake_args=mark_fake_args)
        ...
        if prev_exit_node is not None:
            G.add_edge_if_not_exist(prev_exit_node, cur_entry_node, {"type:TYPE": "FLOWS_TO"})
        ...
        arg_names = filter(lambda x: x is not None, (G.get_name_from_child(param) for param in params))
        args = ",".join(arg_names)
        ...
        if G.exit_when_found and vul_type not in ["proto_pollution", "ipt"]:
            ...
            if len(res_path) != 0:
                with open("vul_func_names.csv", "a") as fp:
                    loggers.main_logger.info(f"{vul_type} successfully found in " f"{G.entry_file_path} at {cur_obj_name}({args})")
...
def call_function(
    ...
    """
    loggers.main_logger.info(
        f"calling function {func_objs}, args: {args}, caller_ast {caller_ast} stmt_id {stmt_id} func_name {func_name} this {this} extra {extra}"
    )
    from src.plugins.manager_instance import internal_manager
    ...
    for i, func_obj in enumerate(func_objs):
        ...
        if func_obj_attrs.get("target_func"):
            ...
            logger.log(
                ATTENTION,
                "Bound function found ({}->{}), this={}".format(func_obj_attrs.get("target_func"), func_obj, _this.obj_nodes),
            )
        ...
        if func_obj in G.internal_objs.values():
            logger.warning("Error: Trying to run an internal obj {} {}" ", skipped".format(func_obj, G.inv_internal_objs[func_obj]))
            continue
        any_func_run = True
        ...
        if python_func:  # special Python function
            if is_new:
                if func_obj in G.builtin_constructors:
                    logger.log(ATTENTION, f"Running Python function {python_func.__name__} {func_obj} ...")
                    try:
                        h = python_func(G, caller_ast, ExtraInfo(extra, branches=next_branches), _this, *_args)
                        created_objs.extend(h.obj_nodes)
                        branch_used_objs = h.used_objs
                    except TypeError as e:
                        logger.error(tb.format_exc())
                else:
                    loggers.main_logger.error(f"Error: try to new Python function {func_obj} {python_func}...")
                    continue
            else:
                logger.log(ATTENTION, f"Running Python function {python_func.__name__} {func_obj} ...")
                try:
                    loggers.main_logger.info(f"python func args: {_args}")
                    h = python_func(G, caller_ast, ExtraInfo(extra, branches=next_branches), _this, *_args)
            ...
            if "labels:label" in func_ast_attr and func_ast_attr["labels:label"] == "Artificial_AST":
                pass
            else:
            ...
            func_scope = G.add_scope(
                "FUNC_SCOPE",
                func_ast,
                f"Function{func_ast}:{caller_ast}",
                func_obj,
                caller_ast,
                func_name,
            ...
            params = G.get_ordered_ast_child_nodes(param_list)
            # add "arguments" array
            arguments_obj = G.add_obj_to_scope(name="arguments", js_type="array", scope=func_scope, ast_node=func_ast)
            j = 0
            while j < len(params) or j < len(_args) or j < 3:
                if j < len(_args):
                    ...
                    for obj in arg_obj_nodes:
                        G.add_obj_as_prop(prop_name=str(j), parent_obj=arguments_obj, tobe_added_obj=obj)
                    ...
                    if j < len(params):
                        ...
                        if G.get_node_attr(param).get("flags:string[]") == "PARAM_VARIADIC":
                            arr = G.add_obj_to_scope(param_name, caller_ast or param, "array", scope=func_scope)
                            length = 0
                            while j < len(_args):
                                logger.debug(f"add arg {param_name}[{length}]" f" <- {arg_obj_nodes}, scope {func_scope}")
                                for obj in arg_obj_nodes:
                                    G.add_obj_as_prop(str(length), parent_obj=arr, tobe_added_obj=obj)
                                j += 1
                                length += 1
                            G.add_obj_as_prop("length", js_type="number", value=length, parent_obj=arr)
                        elif G.get_node_attr(param).get("flags:string[]") == "ObjectPattern":
                            for arg_obj_node in arg_obj_nodes:
                                param_children = G.get_ordered_ast_child_nodes(param)
                                for el in param_children:
                                    r = internal_manager.dispatch_node(
                                        el,
                                        extra=ExtraInfo(parent_obj=arg_obj_node, branches=branches, scope=func_scope),
                                    )
                        else:
                            logger.debug(f"add arg {param_name} <- " f"{arg_obj_nodes}, scope {func_scope}")
                            for obj in arg_obj_nodes:
                                G.add_obj_to_scope(name=param_name, scope=func_scope, tobe_added_obj=obj)
                    else:
                        # this is used to print logs only
                        logger.debug(f"add arg arguments[{j}] <- " f"{arg_obj_nodes}, scope {func_scope}")
                    ...
                    if G.get_node_attr(param).get("flags:string[]") == "PARAM_VARIADIC":
                        ...
                        added_obj = G.add_obj_to_scope(name=param_name, scope=func_scope, ast_node=caller_ast or param, js_type="array")
                        elem = G.add_obj_as_prop(wildcard, caller_ast or param, value=wildcard, parent_obj=added_obj)
                        if mark_fake_args:
                        ...
                        try:
                            ...
                            added_obj = G.add_obj_to_scope(
                                name=param_name,
                                scope=func_scope,
                                ast_node=caller_ast or param,
                                # give __proto__ when checking prototype pollution
                                js_type="object" if G.check_proto_pollution else None,
                                value=wildcard,
                            )
                            for el in param_children:
                                r = internal_manager.dispatch_node(el, extra=ExtraInfo(parent_obj=added_obj, branches=branches, scope=func_scope))
                                object_pattern_added_obj = G.add_obj_to_scope(
                                    name=r.name,
                                    scope=func_scope,
                                    ast_node=caller_ast or param,
                                    js_type="object" if G.check_proto_pollution else None,
                                ...
                                if mark_fake_args:
                                    G.set_node_attr(object_pattern_added_obj, ("tainted", True))
                                    G.set_node_attr(object_pattern_added_obj, ("fake_arg", True))
                        except e:
                            loggers.main_logger.info("handling object pattern param")
                    else:
                        added_obj = G.add_obj_to_scope(
                            name=param_name,
                            scope=func_scope,
                    ...
                    if mark_fake_args:
                        ...
                        logger.debug("{} marked as tainted [3] {}".format(added_obj, func_name))

                        if parent_map_func_objs:
                            add_contributes_to(G, parent_map_func_objs, added_obj)

                    G.add_obj_as_prop(prop_name=str(j), parent_obj=arguments_obj, tobe_added_obj=added_obj)

                    logger.debug(f"add arg {param_name} <- new obj {added_obj}, " f"scope {func_scope}, ast node {param}")
                elif j < 3:
                    ...
                    G.add_obj_as_prop(prop_name=str(j), parent_obj=arguments_obj, tobe_added_obj=added_obj)
                    logger.debug(f"add arguments[{j}] <- new obj {added_obj}, " f"scope {func_scope}, ast node {caller_ast}")
                    if parent_map_func_objs:
                        add_contributes_to(G, parent_map_func_objs, added_obj)
                else:
                    break
                j += 1
            arguments_length_obj = G.add_obj_as_prop(prop_name="length", parent_obj=arguments_obj, value=j, js_type="number")
            ...
            if is_new:
                branch_created_obj, branch_returned_objs = instantiate_obj(G, caller_ast, func_ast, branches=next_branches)
            else:
                backup_objs = G.cur_objs
                if _this:
                ...
                branch_returned_objs, branch_used_objs = simurun_function(G, func_ast, branches=next_branches, caller_ast=caller_ast)
            ...
            if G.get_node_attr(func_ast).get("labels:label") == "Artificial_AST":
                ...
                if fake_returned_obj is None:
                    fake_returned_obj = G.add_obj_node(caller_ast, "object", wildcard)
                ...
                if is_new and branch_created_obj is None:
                    if fake_created_obj is None:
                        fake_created_obj = G.add_obj_node(caller_ast, "object", wildcard)
                ...
                if callback_functions:
                    ...
                    parent_map_func_objs = None
                    loggers.main_logger.info(f"{func_name} calling callback function, {callback_functions}, {func_name}")
                    if func_name.endswith("map") and this and this.obj_nodes:
                        this_obj = this.obj_nodes[0]
                        this_obj_attr = G.get_node_attr(this_obj)
                        if "tainted" in this_obj_attr and this_obj_attr["tainted"] == True:
                            G.set_node_attr(this_obj, ("js_type", "array"))
                            parent_map_func_objs = this.obj_nodes
                            mark_fake_args = True
                    loggers.main_logger.info(
                        f"{func_name} calling callback function, {callback_functions}, {_args}, func_objs: {func_objs}, this: {this}, mark_fake_args: {mark_fake_args}"
                    )
                    call_function(
                        G,
                        callback_functions,
                        ...
                        stmt_id=stmt_id,
                        mark_fake_args=mark_fake_args,
                        parent_map_func_objs=parent_map_func_objs,
                    )
        ...
        # add control flows
        if caller_ast is not None and func_ast is not None and G.get_node_attr(func_ast).get("type") in ["AST_FUNC_DECL", "AST_CLOSURE", "AST_METHOD"]:
            ...
            logger.info(
                ...
                + "CALLS"
                + sty.rs.all
                + " caller_cpg {} -> func_ast {} (Line {} -> Line {}) {}".format(
                    caller_cpg,
                    func_ast,
            ...
            # (called function)
            G.add_edge_if_not_exist(find_function(G, caller_ast), func_ast, {"type:TYPE": "CALLS"})
