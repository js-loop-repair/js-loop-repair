...
def ast_jsx_element(G: Graph, ast_node, extra):
    ...
    if is_react_component:
        ...
        component_def_node_list = G.get_objs_by_name_node(name_node_id=component_name_node)
            ...
            for cur_comp_def_obj in cur_comp_def_obj_list:
                ...
                for p in prototypes:
                    ...
                    if not G.updating_phase:
                        call_function(
                            G, mouting_funcs, extra=extra, args=args, is_new=False, mark_fake_args=False, this=NodeHandleResult(obj_nodes=[class_this_obj])
                        )
                    else:
                        call_function(
                            G, updating_funcs, extra=extra, args=args, is_new=False, mark_fake_args=False, this=NodeHandleResult(obj_nodes=[class_this_obj])
                        )
