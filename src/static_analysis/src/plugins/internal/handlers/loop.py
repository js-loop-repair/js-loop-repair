...
class HandleFor(Handler):
    ...
    def process(self):
        ...
        G.cur_scope = G.add_scope("BLOCK_SCOPE", decl_ast=body, scope_name=G.scope_counter.gets(f"Block{body}"))
        ...
        while True:
            ...
            loggers.main_logger.debug(f"For loop variables: {d}")
            for name, obj_nodes in d.items():
                loggers.main_logger.info(
                    ...
                    + sty.rs.all
                    + ": "
                    + ", ".join([(sty.fg.green + "{}" + sty.rs.all + " {}").format(obj, val_to_str(G.get_node_attr(obj).get("code"))) for obj in obj_nodes])
                )

            # check if the condition is met
            check_result, deterministic = check_condition(G, cond, extra)
            loggers.main_logger.debug(
                "Check condition {} result: possibility: {} deterministic: {}".format(
                    sty.ef.i + G.get_node_attr(cond).get("code") + sty.rs.all, check_result, deterministic
                )
            )
            # avoid infinite loop
            if (not deterministic and counter > 3) or check_result == 0 or counter > 3:
...
class HandleForEach(Handler):
    ...
    def _handle_for_in(self, G, ast_node, extra, logger, handle_node):
        ...
        for obj in handled_obj.obj_nodes:
            ...
            prop_names = G.get_prop_names(obj, exclude_proto=not is_wildcard_obj(G, obj), numeric_only=numeric_only, exclude_wildcard=True)
            ...
            for k in prop_names:
                ...
                logger.debug("For-in loop variables: " f"{sty.ef.i}{handled_key.name}{sty.rs.all}: " f"{sty.fg.green}{key_obj}{sty.rs.all}: {k} from obj {obj}")
                ...
                G.for_stack.append(("for-in", ast_node, G.cur_scope, handled_key.name, k, prop_names, handled_obj.obj_nodes))
                # print(G.for_stack)
                G.assign_obj_nodes_to_name_node(handled_key.name_nodes[0], [key_obj], branches=extra.branches)
                # run the body
    ...
    def _handle_for_of(self, G, ast_node, extra, logger, handle_node):
        ...
        for obj in handled_obj.obj_nodes:
            ...
            # if it's a wildcard object, include "__proto__"
            prop_name_nodes = G.get_prop_name_nodes(obj, exclude_proto=not is_wildcard_obj(G, obj), numeric_only=numeric_only, exclude_wildcard=True)
            if is_wildcard_obj(G, obj):
                # wildcard property for wildcard object
                wildcard_prop_name_node = G.get_prop_name_node(wildcard, obj)
                wildcard_prop_obj_nodes = G.get_obj_nodes(wildcard_prop_name_node, branches=extra.branches)
                if not wildcard_prop_obj_nodes or not wildcard_prop_obj_nodes:
                    wildcard_prop_obj_nodes = [G.add_obj_as_prop(wildcard, ast_node, value=wildcard, parent_obj=obj)]
                    wildcard_prop_name_node = G.get_prop_name_node(wildcard, obj)
                prop_name_nodes.insert(0, wildcard_prop_name_node)
                logger.debug(f"{obj} is a wildcard object.")
            prop_obj_nodes = list(map(lambda nn: G.get_obj_nodes(nn, branches=extra.branches), prop_name_nodes))
            for name_node, obj_nodes in zip(prop_name_nodes, prop_obj_nodes):
                ...
                logger.debug(
                    "For-of loop variables: " f"{sty.ef.i}{handled_value.name}{sty.rs.all}: " f"{sty.fg.green}{k}{sty.rs.all}: {obj_nodes} from obj {obj}"
                )
                # text-based for-stack
                # ----------------------------------------------------------
                # full functional for-stack
                # (type, ast node, scope, loop var name, loop var value,
                #                       loop var value list, loop var origin list)
                G.for_stack.append(("for-of", ast_node, G.cur_scope, handled_value.name, obj_nodes, prop_obj_nodes, handled_obj.obj_nodes))
                # print(G.for_stack)
                G.assign_obj_nodes_to_name_node(handled_value.name_nodes[0], obj_nodes, branches=extra.branches)
                # run the body
                G.last_stmts = [ast_node]
                simurun_block(G, body, branches=extra.branches)
                G.for_stack.pop()
...
class HandleWhile(Handler):
    def process(self):
        ...
        while True:
            ...
            loggers.main_logger.debug(
                "While loop condition {} result: {} {}".format(sty.ef.i + G.get_node_attr(test).get("code") + sty.rs.all, check_result, deterministic)
            )
            # avoid infinite loop
            if (not deterministic and counter > 3) or check_result == 0 or counter > 3:
                loggers.main_logger.debug("For loop {} finished".format(node_id))
