...
class HandleProp(Handler):
    ...
    def process(self):
        ...
        return handle_prop(self.G, self.node_id, side, self.extra)[0]


def handle_prop(G: Graph, ast_node, side=None, extra=ExtraInfo()) -> Tuple[NodeHandleResult, NodeHandleResult]:
    """
    Handle property.
    ...
    if not parent_objs:
        loggers.main_logger.debug("PARENT OBJ {} NOT DEFINED, creating object nodes".format(parent_name))
    ...
    parent_is_tainted = len(list(filter(lambda x: G.get_node_attr(x).get("tainted") is True, parent_objs))) != 0

    parent_is_prop_tainted = (
        len(list(filter(lambda x: G.get_node_attr(x).get("prop_tainted") is True, parent_objs))) != 0
    )
    ...
    for i, prop_name in enumerate(prop_names):
        ...
        name_nodes, obj_nodes, found_in_proto, proto_is_tainted = find_prop(
            G,
            parent_objs,
            prop_name,
            branches,
        ...
        if (
            G.check_ipt
            and side != "left"
            and (proto_is_tainted or (found_in_proto and parent_is_tainted) or parent_is_prop_tainted)
        ):
            ...
            detailed_info = "ipt detected in file {} Line {} node {} type {}".format(
                G.get_ast_node_file_path(ast_node), G.get_node_attr(ast_node).get("lineno:int"), ast_node, ipt_type
            )
...
def find_prop(
    ...
    if in_proto:
        loggers.main_logger.debug('Cannot find "direct" property, going into __proto__ ' f"{parent_objs}...")
    ...
    for parent_obj in parent_objs:
        ...
        # Note that we cannot search "__proto__" under __proto__.
            ...
            __proto__name_node = G.get_prop_name_node("__proto__", parent_obj=parent_obj)
            if __proto__name_node is not None:
                ...
                if parent_obj in __proto__obj_nodes:
                    loggers.main_logger.info(
                        f"__proto__ {__proto__obj_nodes} and " f"parent {parent_obj} have intersection"
                    )
                    __proto__obj_nodes = __proto__obj_nodes.remove(parent_obj)
                if __proto__obj_nodes:
                    __name_nodes, __obj_nodes, __in_proto, __t = find_prop(
                        G,
                        __proto__obj_nodes,
                        prop_name,
                        branches,
            ...
            for t in modeled_builtin_lists:
                ...
                if prop_name in cur_methods:
                    ...
                    return find_prop(
                        ...
                        in_proto=in_proto,
                        depth=depth,
                        prop_name_for_tags=prop_name_for_tags,
                        ast_node=ast_node,
        ...
        if (
            is_wildcard_obj(G, parent_obj)
            and not wc_name_node_found
            and G.get_node_attr(parent_obj)["type"] == "object"
            and (side != "left" or prop_name == wildcard)
        ):
            ...
            # G.add_edge(parent_obj, added_obj, {'type:TYPE': "CONTRIBUTES_TO"})
            if not options.disable_prop_df:
        ...
        # or wildcard property name under normal (known) object
            ...
            (not name_node_found and prop_name != wildcard) or (not wc_name_node_found and prop_name == wildcard)
        ):
            if side == "right":
                ...
                loggers.main_logger.debug(f"Trying to add prop name node under {parent_obj}")
                continue
            else:
