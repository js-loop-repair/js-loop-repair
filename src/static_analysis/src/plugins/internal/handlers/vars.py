...
class HandleVar(Handler):
    ...
    def process(self):
        ...
        return handle_var(self.G, self.node_id, side, self.extra)
...
def handle_var(G: Graph, ast_node, side=None, extra=None):
    ...
    if "namespace" in cur_node_attr:
        ...
        if namespace is not None:
            for n in G.mark_tainted:
                ...
                if (
                    var_name == n["text"]
                    and namespace[0] == n["start"]["row"]
                    and namespace[1] == n["start"]["column"]
                    and namespace[2] == n["end"]["row"]
                    and namespace[3] == n["end"]["column"]
                ):
    ...
    if var_name == "this" and G.cur_objs:
        ...
        return NodeHandleResult(name=var_name, values=[G.get_cur_file_path()], ast_node=ast_node)
    elif var_name == "__dirname":
        return NodeHandleResult(name=var_name, values=[os.path.join(G.get_cur_file_path(), "..")], ast_node=ast_node)
    else:
        now_objs = []
        ...
        if name_node is not None:
            ...
            cur_decl_type = cur_node_attr.get("flags:string[]")
            loggers.main_logger.log(ATTENTION, f"Name node {var_name} not found, create name node, {cur_decl_type}")
            if cur_node_attr.get("flags:string[]") == "JS_DECL_VAR":
                ...
                loggers.main_logger.info(f"added name node {var_name} {name_node} {ast_node}")
        # else:
        #     now_objs = [G.undefined_obj]

    name_nodes = [name_node] if name_node is not None else []

    now_objs = [obj for obj in now_objs if obj is not None]
    ...
    for now_obj in now_objs:
        ...
        new_obj_attr = G.get_node_attr(now_obj)
        logged_obj_attr = {k: (v.__name__ if k == "pythonfunc" and callable(v) else v) for k, v in new_obj_attr.items()}
        loggers.main_logger.info(f"\t{now_obj}: {logged_obj_attr}")

    for name_node in name_nodes:
        G.add_edge(name_node, ast_node, {"type:TYPE": "NAME_TO_AST"})

    return NodeHandleResult(
        obj_nodes=now_objs, name=var_name, name_nodes=name_nodes, ast_node=ast_node  # from_branches=[from_branches],
    )
