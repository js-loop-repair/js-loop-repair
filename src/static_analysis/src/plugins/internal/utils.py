...
def decl_function(G, node_id, func_name=None, obj_parent_scope=None, scope_parent_scope=None):
    """
    ...
    if func_name is not None and func_name != "{closure}":
        G.add_obj_to_scope(name=func_name, scope=obj_parent_scope, tobe_added_obj=added_obj)
        G.add_obj_as_prop("name", node_id, "string", func_name, added_obj)

    param_list = G.get_child_nodes(node_id, edge_type="PARENT_OF", child_type="AST_PARAM_LIST")
...
def decl_vars_and_funcs(G, ast_node, var=True, func=True):
    ...
    for child in children:
        ...
        if var and node_type == "AST_VAR" and G.get_node_attr(child)["flags:string[]"] == "JS_DECL_VAR":
            # var a;
            name = G.get_name_from_child(child)
            if G.get_name_node(name, scope=func_scope, follow_scope_chain=False) is None:
                G.add_obj_to_scope(name=name, scope=func_scope, tobe_added_obj=G.undefined_obj)
        elif var and node_type == "AST_ASSIGN":
            # var a = ...;
            children = G.get_ordered_ast_child_nodes(child)
            if G.get_node_attr(children[0])["type"] == "AST_VAR" and G.get_node_attr(children[0])["flags:string[]"] == "JS_DECL_VAR":
        ...
        elif node_type in ["AST_IF", "AST_IF_ELEM", "AST_FOR", "AST_FOREACH", "AST_WHILE", "AST_SWITCH", "AST_SWITCH_CASE", "AST_EXPR_LIST"]:
...
def to_obj_nodes(G: Graph, handle_result: NodeHandleResult, ast_node=None, incl_existing_obj_nodes=True):
    """
...
def to_values(G: Graph, handle_result: NodeHandleResult, incl_existing_values=True, for_prop=False):
    """
...
def peek_variables(G: Graph, ast_node, extra: ExtraInfo):
    ...
    if G.get_node_attr(ast_node).get("type") == "AST_VAR" or G.get_node_attr(ast_node).get("type") == "AST_NAME":
...
def copy_objs_for_branch(G: Graph, handle_result: NodeHandleResult, branch, ast_node=None, deep=True) -> NodeHandleResult:
    returned_objs = list()
    for obj in handle_result.obj_nodes:
        copied_obj = None
        for e in G.get_in_edges(obj, edge_type="NAME_TO_OBJ"):
            ...
            eb = data.get("branch")
            if name_node in handle_result.name_nodes and (eb is None or (eb.point == branch.point and eb.branch != branch.branch)):
    ...
    return NodeHandleResult(obj_nodes=returned_objs, name=handle_result.name, name_node=handle_result.name_nodes)


def copy_objs_for_parameters(G: Graph, handle_result: NodeHandleResult, ast_node=None, number_of_copies=1, delete_original=True) -> List[List]:
    # deprecated
    returned_objs = list()
    for obj in handle_result.obj_nodes:
        copied_objs = []
...
def to_og_array(G: Graph, array, data, ast_node=None):
    ...
    for i, elem in enumerate(array):
        ...
        for j, obj in enumerate(elem):
            G.add_edge(name_node, obj, {"type:TYPE": "NAME_TO_OBJ", **data[i][j]})
    G.add_obj_as_prop(prop_name="length", ast_node=ast_node, js_type="number", value=len(array), parent_obj=added_array)
    return added_array


def add_contributes_to(G: Graph, sources: Iterable, target, chain_tainted=True):
...
def get_df_callback(G, ast_node=None):
    ...
    return lambda result: build_df_by_def_use(G, cpg_node, result.used_objs, result.jsx_nodes)


# def build_df_of_jsx(G, cur_stmt, jsx_nodes):
#     """
...
#     """
#     if not jsx_nodes or cur_stmt is None:
#         return
...
#     jsx_nodes = list(jsx_nodes)
#     jsx_nodes_set = set(jsx_nodes)
#     for jsx_node in jsx_nodes_set:
#         jsx_ast_node = G.get_jsx_def_ast_node(jsx_node)
#         # print("?", cur_stmt, used_objs, def_ast_node)
#         if jsx_ast_node is None: continue
#         def_cpg_node = G.find_nearest_upper_CPG_node(jsx_ast_node)
#         loggers.main_logger.info(f'jsx ast cpg node {jsx_ast_node} {def_cpg_node} {cur_lineno}')
#         if def_cpg_node is None: continue
#         if def_cpg_node == cur_stmt: continue
#         def_lineno = G.get_node_attr(def_cpg_node).get('lineno:int')
#         loggers.main_logger.info(sty.fg.li_magenta + sty.ef.inverse + "JSX REACHES" + sty.rs.all +
#         " {} -> {} (Line {} -> Line {}), by JSX Node {}".format(def_cpg_node,
#         cur_stmt, def_lineno, cur_lineno, jsx_node))
#         G.add_edge(def_cpg_node, cur_stmt, {'type:TYPE': 'JSX_REACHES', 'jsx_node': jsx_node})

# def build_df_between_obj_and_jsx_node(G, cur_stmt, obj, jsx_node):
#     cur_lineno = G.get_node_attr(cur_stmt).get('lineno:int')
#     jsx_ast_node = G.get_jsx_def_ast_node(jsx_node)
...
#         return
#     def_lineno = G.get_node_attr(def_cpg_node).get('lineno:int')
#     loggers.main_logger.info(sty.fg.li_magenta + sty.ef.inverse + "OBJ_TO_JSX" + sty.rs.all +
#     " {} -> {} (Line {} -> Line {}), by OBJ {}".format(def_cpg_node,
#     cur_stmt, def_lineno, cur_lineno, obj))
#     G.add_edge(def_cpg_node, jsx_ast_node, {'type:TYPE': 'OBJ_TO_JSX', 'obj': obj})


def build_df_by_def_use(G, cur_stmt, used_objs, jsx_nodes=None):
    """
    Build data flows for objects used in current statement.
    The flow will be from the object's definition to current statement (current node).
    """

    # if used_objs and jsx_nodes:
    #     loggers.main_logger.info(f"both objs and jsx_nodes: {cur_stmt} {used_objs} {jsx_nodes}")
    #     if len(used_objs) == 1 and len(jsx_nodes) == 1:
    #         build_df_between_obj_and_jsx_node(G, cur_stmt, used_objs[0], jsx_nodes[0])
    #         return
    #     else:
    #         raise ValueError("Couldn't build the data flow edge between the obj and jsx node.")

    # if not used_objs and jsx_nodes:
    #     build_df_of_jsx(G, cur_stmt, jsx_nodes)
    #     return
    ...
    for obj in used_obj_set:
        ...
        loggers.main_logger.info(
            ...
            + "OBJ REACHES"
            + sty.rs.all
            + " {} -> {} (Line {} -> Line {}), by OBJ {}".format(def_cpg_node, cur_stmt, def_lineno, cur_lineno, obj)
        )
...
def check_condition(G: Graph, ast_node, extra: ExtraInfo):
    ...
    flag = True
    deter_flag = True
    loggers.main_logger.info(f"node_type: {node_type} op_type: {op_type}")
    ...
    if node_type == "AST_BINARY_OP":
        ...
        if op_type == "BINARY_BOOL_OR":
            ...
            build_df_by_def_use(G, ast_node, handled_left.obj_nodes, handled_left.jsx_nodes)
            build_df_by_def_use(G, ast_node, handled_right.obj_nodes, handled_right.jsx_nodes)
    ...
    if not flag:
        ...
        for obj in handled.obj_nodes:
            if obj in [G.undefined_obj, G.null_obj, G.false_obj]:
                true_num += 0.1
                pass
            elif obj in [G.infinity_obj, G.negative_infinity_obj, G.nan_obj, G.true_obj]:
                true_num += 1
            else:
                ...
                if typ == "number":
                    ...
                    if value == wildcard:
                        true_num += 0.5
                        deter_flag = False
                    elif value == "undefined" or value == "null" or value == "false":
                        true_num += 0.1
                        deter_flag = True
                    else:
                        true_num += 1
...
def is_wildcard_obj(G, obj):
    attrs = G.get_node_attr(obj)
    return (attrs.get("type") in ["object", "array"] and attrs.get("code") == wildcard) or (
        attrs.get("type") in ["number", "string"] and attrs.get("code") == wildcard
    )
